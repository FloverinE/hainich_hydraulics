...
lme_psi_midday <- lmer(psi_midday_mpa ~ species * date_fac * year + (1 | sample_id),
                       data = df_psi_midday)
...
emm_psi_midday <- emmeans(lme_psi_midday, ~ species * date_fac * year, 
                          adjust = "bonferroni", 
                          type = "response")
...
df_psi_midday_emmeans <- df_psi_midday |> 
  left_join(df_emm_psi_midday, by = c("species", "date_fac", "year")) |> 
...
ggsave("figures/psi_midday/plot.psi_midday.png", plot.psi_midday.png, width = 8, height = 4, dpi = 150)
...
lme_psi_tlp <- lmer(psi_tlp ~ species * leaf_age_d * year + (leaf_age_d-1 | sample_id), 
                    data = df_pv_params)
...
lme_cap_ft_tot <- lmer(capacitance_ft_tot ~ species * campaign * year + (1 | sample_id), 
                   data = df_pv_params)
lme_cap_ft_tot |> summary()
lme_cap_ft_tot |> r2()

lme_cap_ft_sym <- lmer(capacitance_ft_sym ~ species * campaign * year + (1 | sample_id), 
                       data = df_pv_params)
lme_cap_ft_sym |> summary()
lme_cap_ft_sym |> r2()

lme_cap_tlp_tot <- lmer(capacitance_tlp_tot ~ species * campaign * year + (1 | sample_id), 
                       data = df_pv_params)
lme_cap_tlp_tot |> summary()
lme_cap_tlp_tot |> r2()

lme_cap_tlp_sym <- lmer(capacitance_tlp_sym ~ species * campaign * year + (1 | sample_id), 
                       data = df_pv_params)
...
emm_psi_ft <- emmeans(lme_psi_ft, ~ species * campaign * year, 
                      adjust = "mvt", 
                      trans = "response")
...
emm_psi_tlp <- emmeans(lme_psi_tlp, ~ species * campaign * year, 
                       adjust = "mvt", 
                       type = "response")
...
emm_elast <- emmeans(lme_elast, ~ species * campaign * year, 
                     adjust = "mvt", 
                     type = "response")
...
emm_rwc_tlp <- emmeans(lme_rwc_tlp, ~ species * campaign * year, 
                       adjust = "mvt", 
                       type = "response")
...
df_emm_psi_tlp <- df_emm_psi_tlp |>
  left_join(
    df_pv_params |> dplyr::select(species, campaign, year, date, psi_tlp),
    by = c("species", "campaign", "year")
  ) |>
  mutate(group_y = case_when(species == "FASY" ~ 0, species == "FREX" ~ -4),
         date_plot = date)
...
ggsave("figures/pv/plot.psi_tlp.png", plot.psi_tlp.png, width = 8, height = 4, dpi = 150)
...
glme_gmin <- glmmTMB::glmmTMB(gmin ~ species * campaign * year + (1 | sample_id), 
                              data = df_gmin_params |> filter(rwc_interval == "standard"),
                              family = Gamma(link = "log"))
# glm_test |> summary()
emm_glm <- emmeans(glme_gmin, ~ species * campaign * year,
                   adjust = "mvt", 
                   type = "response")
emm_glm
...
emmeans_gmin_standard <- emmeans(glme_gmin, ~ species * campaign * year,
                                 adjust = "mvt", 
                                 type = "response")
...
plot.gmin_standard.png <- df_emm_gmin_standard |> 
  arrange(species, campaign, year) |>
  mutate(group_y = case_when(species == "FASY" ~ 24, species == "FREX" ~ 0),
         species = recode(species, "FASY" = "Fagus sylvatica", "FREX" = "Fraxinus excelsior"),
         group = f.reletter_cld(.group)) |>
  ggplot() +
...
ggsave("figures/gmin/plot.gmin_standard.png", plot.gmin_standard.png, width = 8, height = 4, dpi = 300)
...
df_ov_params <- read_csv("data/calculated_parameters/df_ov_params.csv") |> 
...
lme_p12 <- lmer(p12 ~ species * campaign * year + (1 | sample_id), 
                data = df_ov_params)

lme_p50 <- lmer(p50 ~ species * campaign * year + (1 | sample_id),
                data = df_ov_params)

lme_p88 <- lmer(p88 ~ species * campaign  * year + (1 | sample_id),
                data = df_ov_params)

## 4.3 emmeans ----

emmeans_p12 <- emmeans(lme_p12, ~ species * campaign * year, 
                       adjust = "bonferroni", 
                       type = "response")


emmeans_p50 <- emmeans(lme_p50, ~ species * campaign * year, 
                       adjust = "bonferroni", 
                       type = "response")



emmeans_p88 <- emmeans(lme_p88, ~ species * campaign * year, 
                       adjust = "bonferroni", 
                       type = "response") 
...
ggsave("figures/ov/plot.p50.png", plot.p50.png, width = 8, height = 4, dpi = 150)
...
lme_t5 <- lmer(estimate ~ species * campaign + (1 | sample_id), 
               data = df_tsm_params |> filter(trait == "T5"))

lme_t50 <- lmer(estimate ~ species * campaign + (1 | sample_id), 
                data = df_tsm_params |> filter(trait == "T50"))

lme_t95 <- lmer(estimate ~ species * campaign + (1 | sample_id),
                data = df_tsm_params |> filter(trait == "T95"))


## 1.5.3 emmeans ----------------------------------------------------------------

emmeans_t5 <- emmeans(lme_t5, ~ species * campaign, 
                      adjust = "mvt", 
                      type = "response")

emmeans_t50 <- emmeans(lme_t50, ~ species * campaign, 
                       adjust = "mvt", 
                       type = "response")

emmeans_t95 <- emmeans(lme_t95, ~ species * campaign,
                       adjust = "mvt", 
                       type = "response")
...
ggsave("figures/tsm/plot.tsm.png", plot.tsm.png, width = 10, height = 4, dpi = 150)
...
lm_psi_md <- lm(
  trait_val ~ leaf_age_d + sum_p_10 + sum_p_et_60 + mean_rew_1 + max_ta_50 + sum_et_35 + mean_ta_60,
