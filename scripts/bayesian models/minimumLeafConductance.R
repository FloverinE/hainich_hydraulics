...
unique(df_gmin_params$date); str(df_gmin_params$date)

# Viz

df_gmin_params %>%
  #mutate(treat = fct_relevel(campaign, "1", "2", "3", "4")) %>%
  ggplot(., aes(x = date, y = gmin, group = species, 
...
gmin.Model.prior1 <- default_prior(gmin ~ species * campaign * year + (1 | sample_id),
                                   family = Gamma(link = "log"),
                                   data = df_gmin_params |> filter(rwc_interval == "standard")
)
gmin.Model.prior2 <- default_prior(gmin ~ species * campaign * year + (1 | sample_id),
...
gmin.Model1 <- brm(gmin ~ species * campaign * year + (1 | sample_id),
                    family = Gamma(link= "log"),
                    data = df_gmin_params |> filter(rwc_interval == "standard"),
                    prior = gmin.Model.prior1,
                    chains = 8,
                    cores = 8,
                    iter = 5000,
                    warmup = 2000,
                    ...
                    save_model = "Model1_gmin",
                    save_pars = save_pars(all = TRUE)
)
gmin.Model2 <- brm(gmin ~ species * campaign * year + (1 | sample_id),
                   family = exponential(link= "log"),
                    data = df_gmin_params |> filter(rwc_interval == "standard"),
                    prior = gmin.Model.prior2,
                    chains = 8,
                    cores = 8,
                    iter = 5000,
                    warmup = 2000,
...
# Model evaluation
summary(gmin.Model1)
gmin.Model1 |> tidybayes::summarise_draws("mean", ~quantile(.x, probs=c(.05, .25, .5, .75, .95)))

coef(gmin.Model1) # Get the coef of the model. The interpretation is the same as the lmer version. But now we have CREDIBLE intervals instead of CONFIDENCE intervals. If the CI contain 0 then the corresponding parameters is statistically not different from 0, else at the 95% level they are not 0.

## Simple plot of the posterior samples of the coefficients and the trace plots (caterplliar looking plots)
plot(gmin.Model2)
mcmc_plot(gmin.Model1, variable = "^b_", regex = T)
mcmc_plot(gmin.Model1, variable = "^b_", regex = T, type = "acf")
...
pp_check(gmin.Model1, ndraws=200, type="loo_ribbon")
...
pp_check(gmin.Model2, ndraws=200, type="loo_pit_qq") 
pp_check(gmin.Model2, ndraws=200, type="scatter_avg") 
...
pp_check(gmin.Model1, ndraws = 100) 

# plot conditional effects for each predictor
plot(conditional_effects(gmin.Model2), ask = FALSE)
...
WAIC(gmin.Model1)
loo_R2(gmin.Model1)
bayes_R2(gmin.Model1)


loo_fit <- loo(gmin.Model1, save_psis = T) # We need the psis for later
loo_fit2 <- loo(gmin.Model2, save_psis = T) # We need the psis for later

loo_compare(loo_fit, loo_fit2) # Model 1 fits better
loo_compare(WAIC(gmin.Model1), WAIC(gmin.Model2))
bayes_factor(gmin.Model1, gmin.Model2, log=T) # Model 1 fits better
