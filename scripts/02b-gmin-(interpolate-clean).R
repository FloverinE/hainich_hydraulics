###############################################################################
#
# extract all the relative water content values in between at greater time resolution
#
...
# export the dataset will be useful later
write_csv(gmin_data2, "outputs/gmin_data2.csv")


# fit with monotonic constraint to extract all the in between points
fit <- gmin_data2 %>%
  group_by(id) %>% 
  mutate(
...
max_gap <- 5 # set the preferred time gap
...
gmin_data_interpolate <- gmin_data2 %>% 
  ...
  select(id, campaign, treatment, real_time,  newtime, leaf_mass_r) %>% 
...
# plot when the values were interpolated 
# mark and verify the interpolated values
gmin_data_predict %>% 
