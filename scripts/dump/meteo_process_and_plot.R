...
df_meteo_2023_2024 <- data.table::fread("data/microclimate/meteo_2023_2024_30min.csv", header = TRUE) |> 
  mutate(
    timestamp = as.POSIXct(timestamp_start, format = "%Y-%m-%d %H:%M:%S"),
...
plot.weekly_temp_2324 <- df_meteo_2023_2024 |> 
  ...
  ggplot() +
  geom_col(
    data = df_meteo_2023_2024 |>
      mutate(year = year(timestamp), month = month(timestamp)) |> 
      group_by(year, month) |> 
      summarise(sum_p = sum(p_1m, na.rm = TRUE) / 5,   # <- nur /2
                timestamp_month = as.Date(first(timestamp))),
    aes(x = timestamp_month, y = sum_p, fill = "monthly sum"),
    alpha = 0.6
  ) +
  geom_vline(data = campaign_dates, aes(xintercept = as.Date(date)), linetype = "dashed") +
  geom_hline(yintercept = 0, col = "grey") +
  geom_ribbon(aes(x = timestamp_new, ymin = min_ta, ymax = max_ta),
              fill = "grey", alpha = 0.5) +
  geom_line(aes(x = timestamp_new, y = mean_ta, color = "weekly mean"), size = 1) +
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(
      ...
      labels = function(x) ifelse(x < 0, "", x)  # don't show negative vals
    )
  ) +
  scale_color_manual(
    name = "Temperature",
...
ggsave("figures/poster/temperature_2324.png", width = 35, height = 15, units = "cm", dpi = 300)
