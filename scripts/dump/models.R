...
lme_psi_midday <- lmer(
  ...
  weights = varPower(form = ~psi_midday_mpa),
  data = df_psi_midday
)
...
ggsave(
  "figures/psi_midday/plot.psi_midday.png",
  plot.psi_midday.png,
  width = 8,
  height = 4,
  dpi = 150
...
## 1.2.1 prepare ------------------------------------------------------------
df_pv_params <- read_csv("data/calculated_parameters/df_pv_params_summ.csv") |>
  mutate(
    # date = as.Date(date),
...
lme_cap_ft_tot <- lmer(
  capacitance_ft_tot ~ species * campaign * year + (1 | sample_id),
  data = df_pv_params
)
...
lme_cap_ft_sym <- lmer(
  capacitance_ft_sym ~ species * campaign * year + (1 | sample_id),
  data = df_pv_params
)
...
lme_cap_tlp_tot <- lmer(
  capacitance_tlp_tot ~ species * campaign * year + (1 | sample_id),
  data = df_pv_params
)
...
lme_cap_tlp_sym <- lmer(
  capacitance_tlp_sym ~ species * campaign * year + (1 | sample_id),
  data = df_pv_params
)
...
df_sugars_summ <- read.csv("data/calculated_parameters/df_sugars_2023.csv") |>
  rbind(read.csv("data/calculated_parameters/df_sugars_2024.csv")) |>
  mutate(
    date = as.Date(date),
    species = as.factor(species),
    sample_id = as.factor(sample_id),
...
df_nutrients_summ <- read.csv(
  "data/calculated_parameters/df_nutrients_2023.csv"
) |>
  mutate(
    date = as.Date(date),
    species = as.factor(species),
...
df_nutrients_nest <- df_nutrients_nest |>
  mutate(
    lme = map(
      data,
      ~ nlme::lme(
        nutrient_conc ~ campaign * species,
...
plot.psi_midday_comb.png <- (plot.psi_midday_meteo.png +
  plot_spacer() +
  plot.psi_midday_coefs.png) +
...
df_hydr_traits_coefs <- df_hydr_traits_nest |>
  ...
  filter(type == FALSE) |>
  select(-c(data, mod.total_sugars, mod.all, type)) |>
