...
psift.Model.prior1 <- default_prior(psi_ft ~ species * campaign * year + (1 | sample_id),
                                  family = student(),
                                  data = df_pv_params)
psift.Model.prior2 <- default_prior(psi_ft ~ species * campaign * year + (1 | sample_id),
...
psift.Model1 <- brm(psi_ft ~ species * campaign * year + (1 | sample_id),
                  family = student(),
                  data = df_pv_params,
                 prior = psift.Model.prior1,
                 chains = 8,
                 cores = 8,
                 iter = 5000,
                 warmup = 2000,
                 ...
                 save_model = "Model1_psift",
                 save_pars = save_pars(all = TRUE)
)
psift.Model2 <- brm(psi_ft ~ species * campaign * year + (1 | sample_id),
                    family = brmsfamily("skew_normal"),
                  data = df_pv_params,
                  prior = psift.Model.prior2,
                  chains = 8,
                  cores = 8,
                  iter = 5000,
                  warmup = 2000,
...
# Model evaluation
summary(psift.Model2)
coef(psift.Model1) # Get the coef of the model. The interpretation is the same as the lmer version. But now we have CREDIBLE intervals instead of CONFIDENCE intervals. If the CI contain 0 then the corresponding parameters is statistically not different from 0, else at the 95% level they are not 0.
tidybayes::summarise_draws(psift.Model2)
brms::fixef(psift.Model1)
## Simple plot of the posterior samples of the coefficients and the trace plots (caterplliar looking plots)
plot(psift.Model1)
mcmc_plot(psift.Model2, variable = "^b_", regex = T)
mcmc_plot(psift.Model1, variable = "^b_", regex = T, type = "acf")
...
pp_check(psift.Model1, ndraws=200, type="loo_ribbon")
...
pp_check(psift.Model2, ndraws=200, type="loo_pit_qq") 
pp_check(psift.Model2, ndraws=200, type="scatter_avg") 
...
pp_check(psift.Model2, ndraws = 100) 

# plot conditional effects for each predictor
plot(conditional_effects(psift.Model2), ask = FALSE)
...
WAIC(psift.Model1)
loo_R2(psift.Model1)
bayes_R2(psift.Model1)


loo_fit <- loo(psift.Model1, save_psis = T) # We need the psis for later
loo_fit2 <- loo(psift.Model2, save_psis = T) # We need the psis for later

loo_compare(loo_fit, loo_fit2)
loo_compare(WAIC(psift.Model1), WAIC(psift.Model2))
bayes_factor(psift.Model1, psift.Model2, log=T) # Model 2 fits better
