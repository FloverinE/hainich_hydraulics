...
tlp_lm1 <- lm(elasticity_tot ~ species * campaign, pv_parameters_clean_stat)

tlp_1 <- lme4::lmer(elasticity_tot ~ species * campaign + 
                (1|tree) + (1|campaign), pv_parameters_clean_stat)


tidy(tlp_1)

tlp_lme <- lmer(psi_tlp ~ species * campaign +
                (1|tree), pv_parameters_clean_stat)


ft_lme <- lmer(psi_ft ~ species * campaign +
                (1|tree), pv_parameters_clean_stat)

rwc_lme <- lmer(rwc_tot_tlp ~ species * campaign +
                  (1|tree), pv_parameters_clean_stat)


elas_lme <- lmer(elasticity_tot ~ species * campaign +
                 (1|tree), pv_parameters_clean_stat)
...
pv_emmean_camp_est <- function(mod) {
  pv_emmean_camp_est <- pv_emmean_camp(mod)$emmeans %>% 
    as.data.frame() %>% 
    as_tibble() %>% 
    select(species, contains("emmean"), campaign, contains("CL")) %>% 
    select(species, contains("emmean"), campaign)  %>% 
...
pv_table_camp <- list(tlp_emmean_camp_tidy,
  ...
  relocate(Parameter, Species, Contrast) %>% 
...
(pv_flex_camp <- pv_table_camp %>% 
    ...
    mutate(across(c('t.ratio'), ~round(., 2)),
           Species = mgsub::mgsub(as.character(Species), c("FASY", "FREX"), c("beech", "ash"))) %>% 
    rename(Campaign = Contrast, t = t.ratio, Significance = signif,
           'P-value' = 'p.value', 'Diff (%)'  = '% diff') %>% 
    flextable() %>% 
    
    mk_par(part = "body", j = c(1), i = c(1:12), value = as_paragraph(as_b("Ψ"),
                                                                     as_sub("TLP"),
                                                                     as_b(" (MPa)"))) %>%
    mk_par(part = "body", j = c(1), i = c(13:24), value = as_paragraph(as_b("Ψ"),
                                                                      as_sub("FT"),
                                                                      as_b(" (MPa)"))) %>%
    mk_par(part = "body", j = c(1), i = c(25:36), value = as_paragraph(as_b("RWC"),
                                                                       as_sub("TLP"),
                                                                       as_b(" (%)"))) %>%
...
pv_plot <- function(mod) {
  emmeans(mod, type = "response", 
    ...
    clean_names() %>% 
    mutate(species = mgsub::mgsub(as.character(species), c("FASY", "FREX"), c("beech", "ash"))) %>% 
 #   mutate(across(c(emmean, lower_cl, upper_cl), funs(. * xx), .names = "{.col}")) %>% 
    ggplot(aes(species, emmean, group = campaign, col = campaign)) +
...
ggsave("figures/2.1-pv-traits.png", height = 100,
