...
df_pv_params %>%
  ...
  ggplot(., aes(x = date, y = elasticity_tot, group = species, 
...
elasTot.Model.prior1 <- default_prior(elasticity_tot ~ species * campaign * year + (1 | sample_id),
                                      family = Gamma(link = "log"),
                                      data = df_pv_params 
)
elasTot.Model.prior2 <- default_prior(elasticity_tot ~ species * campaign * year + (1 | sample_id),
                                   family = gaussian,
                                   data = df_pv_params 
)
...
elasTot.Model1 <- brm(elasticity_tot ~ species * campaign * year + (1 | sample_id),
                   family = Gamma(link= "log"),
                   data = df_pv_params ,
                   prior = elasTot.Model.prior1,
                   chains = 8,
                   cores = 8,
                   iter = 5000,
                   warmup = 2000,
                   ...
                   save_model = "Model1_elasTot.stan",
                   save_pars = save_pars(all = TRUE)
)
elasTot.Model2 <- brm(elasticity_tot ~ species * campaign * year + (1 | sample_id),
                   family = gaussian,
                   data = df_pv_params ,
                   prior = elasTot.Model.prior2,
                   chains = 8,
                   cores = 8,
                   iter = 5000,
                   warmup = 2000,
                   ...
                   save_model = "Model2_elasTot.stan",
                   save_pars = save_pars(all = TRUE)
)

# Model evaluation
summary(elasTot.Model1)
coef(elasTot.Model1) # Get the coef of the model. The interpretation is the same as the lmer version. But now we have CREDIBLE intervals instead of CONFIDENCE intervals. If the CI contain 0 then the corresponding parameters is statistically not different from 0, else at the 95% level they are not 0.

## Simple plot of the posterior samples of the coefficients and the trace plots (caterplliar looking plots)
plot(elasTot.Model1, ask=F)
mcmc_plot(elasTot.Model2, variable = "^b_", regex = T)
mcmc_plot(elasTot.Model2, variable = "^b_", regex = T, type = "acf")
...
pp_check(elasTot.Model2, ndraws=200, type="loo_ribbon")
...
pp_check(elasTot.Model2, ndraws=200, type="loo_pit_qq") 
pp_check(elasTot.Model2, ndraws=200, type="scatter_avg") 
...
pp_check(elasTot.Model2, ndraws = 100) 

# plot conditional effects for each predictor
plot(conditional_effects(elasTot.Model2), ask = FALSE)

## Evaluate model fit. loo_R2 and bayes_R2 are variations of the traditional R^2, but Im not familiar with them so I would not report them in general.
loo_R2(elasTot.Model2)
bayes_R2(elasTot.Model2)


loo_fit <- loo(elasTot.Model1, save_psis = T) # We need the psis for later
loo_fit2 <- loo(elasTot.Model2, save_psis = T) # We need the psis for later

loo_compare(loo_fit, loo_fit2) # Model 1 fits better
loo_compare(WAIC(elasTot.Model1), WAIC(elasTot.Model2))
bayes_factor(elasTot.Model1, elasTot.Model2, log=F) # Model 2 fits better
