...
df_pv_params %>%
  ...
  ggplot(., aes(x = date, y = rwc_tot_tlp, group = species, 
...
rwcTotTtlp.Model.prior1 <- default_prior(rwc_tot_tlp ~ species * campaign * year + (1 | sample_id),
                                      family = weibull(link = "log"),
                                      data = df_pv_params 
)
rwcTotTtlp.Model.prior2 <- default_prior(rwc_tot_tlp ~ species * campaign * year + (1 | sample_id),
                                      family = student,
                                      data = df_pv_params 
)
...
rwcTotTtlp.Model1 <- brm(rwc_tot_tlp ~ species * campaign * year + (1 | sample_id),
                      family = weibull(link= "log"),
                      data = df_pv_params ,
                      prior = rwcTotTtlp.Model.prior1,
                      chains = 8,
                      cores = 8,
                      iter = 5000,
                      warmup = 2000,
                      ...
                      save_model = "Model1_rwcTotTtlp",
                      save_pars = save_pars(all = TRUE)
)
rwcTotTtlp.Model2 <- brm(rwc_tot_tlp ~ species * campaign * year + (1 | sample_id),
                      family = gaussian,
                      data = df_pv_params ,
                      prior = rwcTotTtlp.Model.prior2,
                      chains = 8,
                      cores = 8,
                      iter = 5000,
                      warmup = 2000,
...
# Model evaluation
summary(rwcTotTtlp.Model1)
coef(rwcTotTtlp.Model1) # Get the coef of the model. The interpretation is the same as the lmer version. But now we have CREDIBLE intervals instead of CONFIDENCE intervals. If the CI contain 0 then the corresponding parameters is statistically not different from 0, else at the 95% level they are not 0.

## Simple plot of the posterior samples of the coefficients and the trace plots (caterplliar looking plots)
plot(rwcTotTtlp.Model1)
mcmc_plot(rwcTotTtlp.Model2, variable = "^b_", regex = T)
mcmc_plot(rwcTotTtlp.Model1, variable = "^b_", regex = T, type = "acf")
...
pp_check(rwcTotTtlp.Model2, ndraws=200, type="loo_ribbon")
...
pp_check(rwcTotTtlp.Model2, ndraws=200, type="loo_pit_qq") 
pp_check(rwcTotTtlp.Model2, ndraws=200, type="scatter_avg") 
...
pp_check(rwcTotTtlp.Model2, ndraws = 100) 

# plot conditional effects for each predictor
plot(conditional_effects(rwcTotTtlp.Model2), ask = FALSE)

## Evaluate model fit. loo_R2 and bayes_R2 are variations of the traditional R^2, but Im not familiar with them so I would not repor them in general. FOr more info on the comparison: https://discourse.mc-stan.org/t/loo-r2-documentation-and-comparison-to-bayes-r2/9228/2
WAIC(rwcTotTtlp.Model1)
loo_R2(rwcTotTtlp.Model1)
bayes_R2(rwcTotTtlp.Model1)


loo_fit <- loo(rwcTotTtlp.Model1, save_psis = T) # We need the psis for later
loo_fit2 <- loo(rwcTotTtlp.Model2, save_psis = T) # We need the psis for later

loo_compare(loo_fit, loo_fit2) # Model 1 fits better
loo_compare(WAIC(rwcTotTtlp.Model1), WAIC(rwcTotTtlp.Model2))
bayes_factor(rwcTotTtlp.Model1, rwcTotTtlp.Model2, log=T) # Model 1 fits better
