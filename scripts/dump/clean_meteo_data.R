...
df_meteo_2023_2024 <- data.table::fread(
...
df_meteo_ref_summ_2y <- rbind(df_meteo_ref_summ, df_meteo_ref_summ) |>
  mutate(
    year = c(rep(2023, 12), rep(2024, 12)),
...
df_meteo_2023_2024 |>
  mutate(
    grow_seas_2023 = ifelse(
      timestamp >= "2023-05-01 00:00:00" & timestamp <= "2023-10-31 23:59:59",
      1,
      0
    ),
    grow_seas_2024 = ifelse(
      timestamp >= "2024-05-01 00:00:00" & timestamp <= "2024-10-31 23:59:59",
      1,
      0
    )
  ) |>
...
df_meteo_2023_2024 |>
  ...
  summarise(sum_prec = sum(p_1m), mean_temp = mean(ta_44m, na.rm = TRUE))
...
plot.weekly_temp_2324 <- df_meteo_2023_2024 |>
  ...
  geom_line(
    data = df_meteo_ref_summ_2y,
    aes(x = timestamp, y = mean_ta, col = "monthly reference 2000 - 2022"),
    linetype = "dotted"
  ) +
  scale_x_date(
    breaks = date_breaks,
  ...
  scale_color_manual(
    values = c(
      "weekly maximum to minimum" = "grey",
      "weekly mean" = "black",
  ...
  plot_theme +
  theme(
    legend.position = c(0.98, 0.02), # Move legend snugly into the bottom right
...
plot.daily_precipitation_2324 <- df_meteo_2023_2024 |>
  ...
  filter(
    timestamp >= as.POSIXct("2023-01-01 00:30:00") &
      timestamp <= as.POSIXct("2024-12-31 23:59:59")
  ) |>
  ggplot() +
  geom_vline(
    data = campaign_dates,
    aes(xintercept = as.Date(date), col = "campaign"),
    linetype = "dashed"
  ) +
...
plot.monthly_precipitation_2324 <- df_meteo_2023_2024 |>
  ...
  ggplot() +
  geom_col(
    aes(x = timestamp_new, y = sum_p, fill = "monthly sum"),
  ...
  geom_col(
    data = df_meteo_ref_summ_2y,
    aes(x = timestamp, y = sum_prec, fill = "monthly reference 2000 - 2022"),
    col = "black",
    position = position_nudge(x = 5),
    width = 10
  ) +
...
plot.daily_mean_max_vpd_2324 <- df_meteo_2023_2024 |>
  ...
  plot_theme +
  theme(
    legend.position = c(0.98, 0.98), # Move legend snugly into the bottom right
...
plot.weekly_vpd_2324 <- df_meteo_2023_2024 |>
  ...
  geom_line(
    data = df_meteo_ref_summ_2y,
    aes(x = timestamp, y = mean_vpd, col = "monthly reference 2000 - 2022"),
    linetype = "dotted"
  ) +
  scale_x_date(
    breaks = date_breaks,
...
plot.daily_rew_2324 <- df_meteo_2023_2024 |>
  ...
  plot_theme +
  theme(
    legend.position = c(0.02, 0.02), # Move legend to the bottom left
...
plot.daily_ppfd_2324 <- df_meteo_2023_2024 |>
  ...
  plot_theme +
  theme(
    legend.position = c(0.02, 0.98), # Move legend snugly into the bottom right
...
plot.daily_etpet_2324 <- df_meteo_2023_2024 |>
  ...
  group_by(day) |>
  summarise(
    et_pet_mm = sum(et_pet_mm, na.rm = TRUE),
...
plot.monthly_etpet_2324 <- df_meteo_2023_2024 |>
  ...
  summarise(
    et_mm = sum(et_mm, na.rm = TRUE),
    pet_mm = sum(pet_mm, na.rm = TRUE),
    timestamp = as.Date(first(timestamp)),
    .groups = "drop"
  ) |> # Drop grouping warning
  ggplot() +
  geom_col(
    aes(x = timestamp, y = et_mm, fill = "Evapotranspiration"),
    position = position_nudge(x = -5),
    width = 10,
    col = "black"
  ) +
  geom_col(
    aes(x = timestamp, y = pet_mm, fill = "Potential Evapotranspiration"),
    position = position_nudge(x = 5),
    width = 10,
    col = "black"
  ) +
...
ggsave(
  plot.meteo_2023_2024_1.png,
  file = "figures/meteo/plot.meteo_2023_2024_1.png",
  width = 16,
  height = 15
)
...
ggsave(
  plot.meteo_2023_2024_2.png,
  file = "figures/meteo/plot.meteo_2023_2024_2.png",
  width = 16,
  height = 15
)
...
df_meteo_2000_2024 <- data.table::fread(
  "data/microclimate/meteo_flux_2000_2024.csv",
  header = TRUE
)
...
df_fluxes_2023_2024 <- data.table::fread(
  "data/microclimate/fluxes_2023_2024_30min.csv",
  header = TRUE
) |>
...
## new data 09.03.
df_meteo_30min_2023 <- data.table::fread(
  "data/microclimate/DE-Hai_Meteo_tower_30min_20230101_20240101_gapfilled.csv",
  skip = 2
)
names(df_meteo_30min_2023) = data.table::fread(
  "data/microclimate/DE-Hai_Meteo_tower_30min_20230101_20240101_gapfilled.csv",
  header = F
)[1, ] |>
  unlist()
...
df_meteo_30min_2024 <- data.table::fread(
  "data/microclimate/DE-Hai_Meteo_tower_30min_20240101_20250101_gapfilled.csv",
  skip = 2
)
names(df_meteo_30min_2024) = data.table::fread(
  "data/microclimate/DE-Hai_Meteo_tower_30min_20240101_20250101_gapfilled.csv",
  header = F
)[1, ] |>
  unlist()
...
df_fluxes_30min_2023 <- data.table::fread(
  "data/microclimate/DE-Hai_HS_LI7200_fluxes_30min_20230101_20240101.csv",
  skip = 2
)
names(df_fluxes_30min_2023) = data.table::fread(
  "data/microclimate/DE-Hai_HS_LI7200_fluxes_30min_20230101_20240101.csv",
  header = F
)[1, ] |>
  unlist()
...
df_fluxes_30min_2024 <- data.table::fread(
  "data/microclimate/DE-Hai_HS_LI7200_fluxes_30min_20240101_20250101.csv",
  skip = 2
)
names(df_fluxes_30min_2024) = data.table::fread(
  "data/microclimate/DE-Hai_HS_LI7200_fluxes_30min_20240101_20250101.csv",
  header = F
)[1, ] |>
  unlist()
...
df_meteo_2000_2023_join <- df_meteo_2000_2023 |>
  ...
  mutate(
    lw_out_44m = case_when(lw_out_44m < -1000 ~ NA, TRUE ~ lw_out_44m),
...
df_meteo_2000_2024 <- rbind(
  df_meteo_2000_2023_join,
  df_meteo_2023_2024 |> filter(timestamp > "2023-12-20"),
  fill = T
) |>
  mutate(
    pet_mm = case_when(pet_mm < 0 ~ 0, TRUE ~ pet_mm),
...
### 30 mins ----
meteo_2023_30min <- read.csv(
  "data/microclimate/DE-Hai_Meteo_tower_30min_20230101_20231231_gapfilled.csv",
  header = F,
  skip = 2
)

meteo_2024_30min <- read.csv(
  "data/microclimate/DE-Hai_Meteo_tower_30min_20240101_20241031_gapfilled.csv",
  header = F,
  skip = 2
)

colnames(meteo_2023_30min) <- read.csv(
  "data/microclimate/DE-Hai_Meteo_tower_30min_20230101_20231231_gapfilled.csv",
  header = F,
  nrows = 1
)
colnames(meteo_2024_30min) <- read.csv(
  "data/microclimate/DE-Hai_Meteo_tower_30min_20240101_20241031_gapfilled.csv",
  header = F,
  nrows = 1
)
...
meteo_2023_30min <- meteo_2023_30min[,
  colnames(meteo_2023_30min) %in% colnames(meteo_2024_30min)
]
meteo_2024_30min <- meteo_2024_30min[,
  colnames(meteo_2024_30min) %in% colnames(meteo_2023_30min)
]

meteo_2023_2024_30min <- rbind(meteo_2023_30min, meteo_2024_30min)

write.csv(
  meteo_2023_2024_30min,
  "data/microclimate/meteo_2023_2024_30min.csv",
  row.names = FALSE
)
...
meteo_2023_10min <- read.csv(
  "data/microclimate/DE-Hai_Meteo_tower_10min_20230101_20231231_gapfilled.csv",
  header = F,
  skip = 2
)
meteo_2024_10min <- read.csv(
  "data/microclimate/DE-Hai_Meteo_tower_10min_20240101_20241031_gapfilled.csv",
  header = F,
  skip = 2
)
...
### hourly ----
meteo_2023_H <- read.csv(
  "data/microclimate/DE-Hai_Meteo_tower_H_20230101_20231231_gapfilled.csv",
  header = T
)
...
meteo_2024_H <- read.csv(
  "data/microclimate/DE-Hai_Meteo_tower_H_20240101_20241031_gapfilled.csv",
  header = F,
  skip = 2
)

colnames(meteo_2024_H) <- read.csv(
  "data/microclimate/DE-Hai_Meteo_tower_30min_20240101_20241031_gapfilled.csv",
  header = F,
  nrows = 1
)
...
colnames(meteo_2023_H)[which(
  !colnames(meteo_2023_H) %in% colnames(meteo_2024_H)
)]
...
meteo_2024_H <- meteo_2024_H[,
  colnames(meteo_2024_H) %in% colnames(meteo_2023_H)
]

df_meteo_H <- rbind(meteo_2023_H, meteo_2024_H)
write.csv(
  df_meteo_H,
  "data/microclimate/meteo_2023_2024_H.csv",
  row.names = FALSE
)

meteo_2023 <- read.csv(
  "data/Meteorology_tower/DE-Hai_Meteo_all_D_20230101_20231107_gapfilled.csv",
  header = T
)
...
meteo_2024 <- read.csv(
  "data/Meteorology_tower/DE-Hai_Meteo_tower_D_20240101_20241031_gapfilled.csv",
  header = F,
  skip = 2
)
colnames(meteo_2024) <- read.csv(
  "data/Meteorology_tower/DE-Hai_Meteo_tower_D_20240101_20241031_gapfilled.csv",
  header = F,
  nrows = 1
)
...
write.csv(
  df_meteo,
  "data/microclimate/meteo_2023_2024_D.csv",
  row.names = FALSE
)
...
write.csv(
  df_meteo_2000_2023,
  "data/Meteorology_Tower/meteo_2000_2023.csv",
  row.names = FALSE
)
...
### 30 mins ----
fluxes_2023_H <- read.csv(
  "data/microclimate/DE-Hai_HS_LI7200_fluxes_30min_20230101_20231001.csv",
  header = T,
  skip = 2
)
colnames(fluxes_2023_H) <- read.csv(
  "data/microclimate/DE-Hai_HS_LI7200_fluxes_30min_20230101_20231001.csv",
  header = F,
  nrows = 1
)
...
fluxes_2024_H <- read.csv(
  "data/microclimate/DE-Hai_HS_LI7200_fluxes_30min_20240101_20241001.csv",
  header = T,
  skip = 2
)
colnames(fluxes_2024_H) <- read.csv(
  "data/microclimate/DE-Hai_HS_LI7200_fluxes_30min_20240101_20241001.csv",
  header = F,
  nrows = 1
)
...
fluxes_2023_H <- fluxes_2023_H[,
  colnames(fluxes_2023_H) %in% colnames(fluxes_2024_H)
]
fluxes_2024_H <- fluxes_2024_H[,
  colnames(fluxes_2024_H) %in% colnames(fluxes_2023_H)
]
...
fluxes_2023 <- read.csv(
  "data/EddyCovarianceFluxes_tower/DE-Hai_HS_LI7200_fluxes_daily_20230101_20231001.csv",
  header = T,
  skip = 1
)
colnames(fluxes_2023) <- read.csv(
  "data/EddyCovarianceFluxes_tower/DE-Hai_HS_LI7200_fluxes_daily_20230101_20231001.csv",
  header = F,
  nrows = 1
)
...
fluxes_2024 <- read.csv(
  "data/EddyCovarianceFluxes_tower/DE-Hai_HS_LI7200_fluxes_daily_20240101_20241001.csv",
  header = T,
  skip = 1
)
colnames(fluxes_2024) <- read.csv(
  "data/EddyCovarianceFluxes_tower/DE-Hai_HS_LI7200_fluxes_daily_20240101_20241001.csv",
  header = F,
  nrows = 1
)
...
write.csv(
  df_fluxes_2023_2024_D,
  "data/fluxes_2023_2024_D.csv",
  row.names = FALSE
)
...
f.delta = function(T_air) {
  delta <- 4098 *
    (0.6108 * exp((17.27 * T_air) / (T + 237.3))) /
    (T_air + 237.3)^2
}
...
plot(
  y = df_meteo_flux_30min$ta_44m,
  x = df_meteo_flux_30min$timestamp,
  type = "l"
)
...
df_swc <- df_meteo_fluxes_2023_2024 |>
  ...
  group_by(timestamp) |>
  summarise(
    swc_8cm = mean(c(swc_8cm_1, swc_8cm_2, swc_8cm_3, swc_8cm_4), na.rm = TRUE),
...
min_swc <- quantile(df_swc_longterm$swc_0_32_avg_d, probs = c(0.02)) |>
  as.numeric()
max_swc <- quantile(df_swc_longterm$swc_0_32_avg_d, probs = c(0.98)) |>
  as.numeric()
...
df_swc <- df_swc |>
  mutate(
    rew = f.rew(swc_0_32_avg, min_swc, max_swc),
    rew_per = rew * 100,
...
df_hainich_meteo_summ <- df_hainich_meteo_summ |>
  mutate(
    pet_tw_mm = thornthwaite(mean_ta_degc, 51.1),
...
climate_hainich_month <- ts(
  df_hainich_meteo_summ,
  start = c(2000, 1),
  frequency = 12
)
...
df_hainich_meteo_summ <- df_hainich_meteo_summ |>
  mutate(
    spei_3mon = spei_hainich_3mon$fitted |> unlist(),
...
write.csv(
  df_hainich_meteo_summ,
  "data/microclimate/df_hainich_meteo_summ.csv",
  row.names = F
)
...
plot.spei3month.png <- df_hainich_meteo_summ |>
  ...
  geom_line(
    aes(y = bal_fao_mm, col = "Precipitation balance"),
    show.legend = F
  ) +
...
plot.spei3_12month.png <- plot.spei3month.png +
  plot.spei12month.png +
  plot_layout(heights = c(6, 6))
plot.spei3_12month.png

ggsave(
  "figures/meteo/plot.spei3_12month.png",
  plot.spei3_12month.png,
  width = 16,
  height = 12,
  units = "cm"
...
plot(spei(
  ts(wichita[, "BAL"], freq = 12, start = c(1980, 6)),
  12,
  ref.start = c(1980, 1),
  ref.end = c(2000, 1)
))
...
dwd_eisenach_tair_hist <- read.table(
  "data/dwd/t_air/produkt_tu_stunde_20071101_20231231_07368.txt",
  header = TRUE,
  sep = ";"
) |>
  mutate(
...
dwd_eisenach_tair_rec <- read.table(
  "data/dwd/t_air/produkt_tu_stunde_20230828_20250227_07368.txt",
  header = TRUE,
  sep = ";"
) |>
  mutate(
...
dwd_eisenach_prec_hist <- read.table(
  "data/dwd/prec/produkt_rr_stunde_20071101_20231231_07368.txt",
  header = TRUE,
  sep = ";"
) |>
  mutate(
...
dwd_eisenach_prec_rec <- read.table(
  "data/dwd/prec/produkt_rr_stunde_20230828_20250227_07368.txt",
  header = TRUE,
  sep = ";"
) |>
  mutate(
...
climate_eisenach_month <- climate_eisenach_2007_24 |>
  mutate(year = year(timestamp), month = month(timestamp)) |>
  group_by(year, month) |>
  summarise(sum_prec_mm = sum(prec_mm), mean_ta_degc = mean(ta_degc))

climate_eisenach_month <- climate_eisenach_month |>
  filter(year != 2007) |>
  mutate(
    pet_mm = thornthwaite(mean_ta_degc, 51.1),
...
climate_eisenach_spei_12mon = spei(
  climate_eisenach_month[, "bal_mm"],
  scale = 12
)
