...
lme_psi_midday <- lmer(psi_midday_mpa ~ species * date_fac + (1 | sample_id),
                       data = df_psi_midday)
...
df_psi_midday |> 
  ...
  geom_line(aes(x = doy, y = predict(lme_psi_midday), color = species, 
...
emm_psi_midday <- emmeans(lme_psi_midday, ~ species * date_fac, 
                          adjust = "bonferroni", 
                          type = "response")
...
df_psi_midday_emmeans <- df_psi_midday |> 
  ...
  mutate(group_y = case_when(species == "FASY" ~ 0,
                             species == "FREX" ~ -4),
         date_plot = date)
...
plot.psi_midday.png <- df_psi_midday_emmeans |> 
  ...
  ggplot() +
  geom_point(aes(x = date_plot, y = psi_midday_mpa, col = species), alpha = plot_alpha,
             position = position_dodge(width = plot_position_offset)) +
  geom_line(aes(x = date_plot, y = emmean, col = species), alpha = 0.2,
            position = position_dodge(width = plot_position_offset)) +
  geom_errorbar(aes(x = date_plot, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
                position = position_dodge(width = plot_position_offset), alpha = 0.2) +
  geom_text(aes(x = date_plot, y = group_y, label = group, col = species), show.legend = F) +
...
ggsave("figures/psi_midday/plot.psi_midday_datefac.png", plot.psi_midday.png, width = 8, height = 4, dpi = 150)

df_psi_midday_emmeans |> 
  arrange(psi_midday_mpa) |> 
  dplyr::select(c(species, year, sample_id, campaign, emmean, psi_midday_mpa)) |> 
  mutate(psi_midday_mpa = psi_midday_mpa |> round(1)) |> 
  View()

df_psi_midday_emmeans |> 
  group_by(species, year, campaign) |> 
  summarise(mean = mean(psi_midday_mpa) |> round(1),
...
lme_psi_midday |> broom.mixed::tidy() |> 
  write.table("clipboard", sep = "\t", row.names = F)
...
lme_psi_ft <- lmer(psi_ft ~ species * date_fac * year + (1 | sample_id), 
                   data = df_pv_params)
...
ggplot(df_pv_params) +
  ...
  geom_line(aes(x = doy, y = predict(lme_psi_ft), color = species, group = sample_id)) +
  facet_wrap(~ year, scales = "free_x") +
  thesis_theme
...
lme_psi_tlp <- lmer(psi_tlp ~ species * date_fac * year + (1 | sample_id), 
                    data = df_pv_params)
...
ggplot(df_pv_params) +
  ...
  geom_line(aes(x = date, y = predict(lme_psi_tlp), color = species, group = sample_id)) +
  facet_wrap(~ year, scales = "free_x") +
  thesis_theme
...
lme_elast <- lmer(elast_tot ~ species * date_fac * year + (leaf_age_d | sample_id), 
                  data = df_pv_params)
...
ggplot(df_pv_params) +
  ...
  geom_line(aes(x = date, y = predict(lme_elast), color = species, group = sample_id)) +
  facet_wrap(~ year, scales = "free_x") +
  thesis_theme

lme_rwc_tlp <- lmer(rwc_tot_tlp ~ species * date_fac * year + (1 | year/sample_id), data = df_pv_params)
lme_rwc_tlp |> summary()
lme_rwc_tlp |> r2()

ggplot(df_pv_params) +
  geom_point(aes(x = date, y = rwc_tot_tlp, color = species)) +
  geom_line(aes(x = date, y = predict(lme_rwc_tlp), color = species, group = sample_id)) +
  facet_wrap(~ year, scales = "free_x") +
  thesis_theme

lme_cap_ft_tot <- lmer(capacitance_ft_tot ~ species * date_fac * year + (1 | sample_id), 
                       data = df_pv_params)
lme_cap_ft_tot |> summary()
lme_cap_ft_tot |> r2()

lme_cap_ft_sym <- lmer(capacitance_ft_sym ~ species * date_fac * year + (1 | sample_id), 
                       data = df_pv_params)
lme_cap_ft_sym |> summary()
lme_cap_ft_sym |> r2()

lme_cap_tlp_tot <- lmer(capacitance_tlp_tot ~ species * date_fac * year + (1 | sample_id), 
                        data = df_pv_params)
lme_cap_tlp_tot |> summary()
lme_cap_tlp_tot |> r2()

lme_cap_tlp_sym <- lmer(capacitance_tlp_sym ~ species * date_fac * year + (1 | sample_id), 
                        data = df_pv_params)
...
ggplot(df_pv_params) +
  ...
  geom_line(aes(x = date, y = predict(lme_cap_ft_tot), color = species, group = sample_id)) +
  facet_wrap(~ year, scales = "free_x") +
  thesis_theme
...
ggplot(df_pv_params) +
  ...
  geom_line(aes(x = date, y = predict(lme_cap_tlp_sym), color = species, group = sample_id)) +
  facet_wrap(~ year, scales = "free_x") +
  thesis_theme
...
ggplot(df_pv_params) +
  geom_point(aes(x = date, y = capacitance_ft_tot / capacitance_tlp_tot, color = species))

df_pv_params$capacitance_ft_tot / df_pv_params$capacitance_tlp_tot

## 1.2.3 emmeans ----------------------------------------------------------------

emm_psi_ft <- emmeans(lme_psi_ft, ~ species * date_fac * year, 
                      adjust = "mvt", 
                      trans = "response")
...
emm_psi_tlp <- emmeans(lme_psi_tlp, ~ species * date_fac * year, 
                       adjust = "mvt", 
                       type = "response")
...
emm_elast <- emmeans(lme_elast, ~ species * date_fac * year, 
                     adjust = "mvt", 
                     type = "response")
...
emm_rwc_tlp <- emmeans(lme_rwc_tlp, ~ species * date_fac * year, 
                       adjust = "mvt", 
                       type = "response")
...
df_emm_psi_ft <- df_emm_psi_ft |>
  ...
  mutate(group_y = case_when(species == "FASY" ~ 0, species == "FREX" ~ -4),
         date_plot = date)
...
plot.psi_ft.png <- df_emm_psi_ft |> 
  ...
  mutate(species = recode(species, "FASY" = "Fagus sylvatica", "FREX" = "Fraxinus excelsior"),
         group = f.reletter_cld(.group)) |>
  ggplot() +
  geom_point(aes(x = date_plot, y = psi_ft, col = species), alpha = plot_alpha,
             position = position_dodge(width = plot_position_offset), show.legend = F) +
  geom_line(aes(x = date_plot, y = emmean, col = species), alpha = 0.2,
            position = position_dodge(width = plot_position_offset), show.legend = F) +
  geom_errorbar(aes(x = date_plot, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
                position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = F) +
  geom_text(aes(x = date_plot, y = group_y, label = group, col = species), show.legend = F) +
  scale_color_oi(order = c(6, 2)) +
  scale_x_date(limits = c(as.Date("2000-05-15"), as.Date("2000-09-30"))) +
  lims(y = c(-4, 0)) +
...
ggsave("figures/pv/plot.psi_ft.png", plot.psi_ft.png, width = 8, height = 4, dpi = 150)
...
df_emm_psi_ft |> 
  group_by(species, year, campaign) |> 
  summarise(mean = mean(psi_ft) |> round(1),
...
plot.psi_tlp.png <- df_emm_psi_tlp |> 
  ...
  mutate(species = recode(species, "FASY" = "Fagus sylvatica", "FREX" = "Fraxinus excelsior"),
         group = f.reletter_cld(.group)) |>
  ggplot() +
  geom_point(aes(x = date_plot, y = psi_tlp, col = species), alpha = plot_alpha,
             position = position_dodge(width = plot_position_offset), show.legend = F) +
  geom_line(aes(x = date_plot, y = emmean, col = species), alpha = 0.2,
            position = position_dodge(width = plot_position_offset), show.legend = F) +
  geom_errorbar(aes(x = date_plot, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
...
ggsave("figures/pv/plot.psi_tlp_datefac.png", plot.psi_tlp.png, width = 8, height = 4, dpi = 150)
...
df_emm_psi_tlp |> 
  group_by(species, year, campaign) |> 
  summarise(mean = mean(psi_tlp) |> round(1),
...
df_emm_elast <- df_emm_elast |>
  left_join(
    df_pv_params |> dplyr::select(species, campaign, year, date, elasticity_tot),
...
plot.elast.png <- df_emm_elast |> 
  arrange(species, campaign, year) |>
  mutate(group_y = case_when(species == "FASY" ~ 40, species == "FREX" ~ 0),
         species = recode(species, "FASY" = "Fagus sylvatica", "FREX" = "Fraxinus excelsior"),
         group = f.reletter_cld(.group)) |>
  ggplot() +
  geom_point(aes(x = date_plot, y = elasticity_tot, col = species), alpha = plot_alpha,
             position = position_dodge(width = plot_position_offset), show.legend = F) +
  geom_line(aes(x = date_plot, y = emmean, col = species), alpha = 0.2,
            position = position_dodge(width = plot_position_offset), show.legend = F) +
  geom_errorbar(aes(x = date_plot, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
...
ggsave("figures/pv/plot.elast.png", plot.elast.png, width = 8, height = 4, dpi = 150)
...
df_emm_elast |> 
  group_by(species, year, campaign) |> 
  summarise(mean = mean(elasticity_tot  ) |> round(1),
...
plot.rwc_tlp.png <- df_emm_rwc_tlp |> 
  arrange(species, campaign, year) |>
  mutate(group_y = case_when(species == "FASY" ~ 100, species == "FREX" ~ 65),
         species = recode(species, "FASY" = "Fagus sylvatica", "FREX" = "Fraxinus excelsior"),
         group = f.reletter_cld(.group)) |>
  ggplot() +
  geom_point(aes(x = date_plot, y = rwc_tot_tlp, col = species), alpha = plot_alpha,
             position = position_dodge(width = plot_position_offset), show.legend = T) +
  geom_line(aes(x = date_plot, y = emmean, col = species), alpha = 0.2,
            position = position_dodge(width = plot_position_offset), show.legend = T) +
  geom_errorbar(aes(x = date_plot, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
                position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = T) +
  geom_text(aes(x = date_plot, y = group_y, label = group, col = species), show.legend = F) +
  scale_color_oi(order = c(6, 2)) +
  scale_x_date(limits = c(as.Date("2000-05-15"), as.Date("2000-09-30"))) +
  lims(y = c(65, 100)) +
...
ggsave("figures/pv/plot.rwc_tlp.png", plot.rwc_tlp.png, width = 8, height = 4, dpi = 150)
...
df_emm_rwc_tlp |> 
  group_by(species, year, campaign) |> 
  summarise(mean = mean(rwc_tot_tlp) |> round(1),
            sd = sd(rwc_tot_tlp) |> round(1),
            emmean = emmean) |> 
  View()

extra_theme_pv <- theme(axis.title.x = element_blank(),
                        axis.title.y = element_text(size = 8),
...
plot.all_PV.png <- 
  plot.psi_ft.png +extra_theme_pv+
  plot.psi_tlp.png +extra_theme_pv+
  plot.elast.png +extra_theme_pv+
  plot.rwc_tlp.png +
  plot_layout(heights = c(5,5,5,5))
plot.all_PV.png

ggsave("figures/pv/plot.all_PV.png", plot.all_PV.png, width = 16, height = 23, dpi = 150, unit = "cm")

## 1.2.5 export ------------------------------------------------------------------
lme_psi_ft |> broom.mixed::tidy() |> 
  write.table("clipboard", sep = "\t", row.names = F)
...
lme_psi_ft |> r2() |> as.data.frame() |>
  write.table("clipboard", sep = "\t", row.names = F)

lme_psi_tlp |> broom.mixed::tidy() |> 
  write.table("clipboard", sep = "\t", row.names = F)
...
lme_psi_tlp |> r2() |> as.data.frame() |>
  write.table("clipboard", sep = "\t", row.names = F)

lme_elast |> broom.mixed::tidy() |> 
  write.table("clipboard", sep = "\t", row.names = F)
...
lme_elast |> r2() |> as.data.frame() |>
  write.table("clipboard", sep = "\t", row.names = F)

lme_rwc_tlp |> broom.mixed::tidy() |> 
  write.table("clipboard", sep = "\t", row.names = F)
...
df_gmin_params <- read.csv("data/calculated_parameters/df_gmin_params.csv") |>
  ...
  mutate(date = case_when(date == "2023-07-19" ~ "2023-07-17" |> as.Date(),
...
glme_gmin <- glmmTMB::glmmTMB(gmin ~ species * date_fac * year + (1 | sample_id), 
                              data = df_gmin_params |> filter(rwc_interval == "standard"),
                              family = Gamma(link = "log"))
# glm_test |> summary()
emm_glm <- emmeans(glme_gmin, ~ species * date_fac * year,
                   adjust = "mvt", 
                   type = "response")
emm_glm

df_gmin_params |> filter(rwc_interval == "standard") |> 
  ggplot() +
  geom_point(aes(x = date, y = gmin, color = species)) +
  geom_line(aes(x = date, y = predict(glm_test, type = "response"), color = species, group = sample_id)) +
  facet_wrap(~ year, scales = "free_x") +
  thesis_theme
...
emmeans_gmin_standard <- emmeans(glme_gmin, ~ species * date_fac * year,
                                 adjust = "mvt", 
                                 type = "response")
...
plot.gmin_standard.png <- df_emm_gmin_standard |> 
  arrange(species, date_fac, year) |>
  mutate(group_y = case_when(species == "FASY" ~ 20, species == "FREX" ~ 0),
         species = recode(species, "FASY" = "Fagus sylvatica", "FREX" = "Fraxinus excelsior"),
         group = f.reletter_cld(.group)) |>
  ggplot() +
  geom_point(aes(x = date_plot, y = gmin, col = species), alpha = plot_alpha,
             position = position_dodge(width = plot_position_offset)) +
  geom_line(aes(x = date_plot, y = response, col = species), alpha = 0.2,
            position = position_dodge(width = plot_position_offset)) +
  geom_errorbar(aes(x = date_plot, ymin = asymp.LCL, ymax = asymp.UCL, color = species), width = 3,
                position = position_dodge(width = plot_position_offset), alpha = 0.2) +
  geom_text(aes(x = date_plot, y = group_y, label = group, col = species), show.legend = F) +
  scale_color_oi(order = c(6, 2)) +
  scale_x_date(limits = c(as.Date("2000-05-15"), as.Date("2000-09-30"))) +
  labs(y = expression(g[min] ~ "["*mmol~m^{-2}~s^{-1}*"]"), x = "Date") +
  lims(y = c(0, 20)) +
  guides(color = guide_legend(title = "Species")) +
  facet_wrap(~ year) +
  thesis_theme +
...
ggsave("figures/gmin/plot.gmin_datefac.png", plot.gmin_standard.png, width = 8, height = 4, dpi = 300)
...
df_emm_gmin_standard |> 
  group_by(species, year, campaign) |> 
  summarise(mean = mean(gmin) |> round(1),
            sd = sd(gmin) |> round(1),
...
df_emm_gmin_standard |> 
  arrange(year, species, campaign) |> 
  mutate(.group = f.reletter_cld(.group)) |> 
  select(c(species, campaign, year, response, SE, df, asymp.LCL, asymp.UCL, .group)) |>
  distinct() |> 
  write.table("clipboard", sep = "\t", row.names = F, na = "")

glme_gmin |> broom.mixed::tidy() |> 
  write.table("clipboard", sep = "\t", row.names = F, na = "")
...
df_ov_params <- read.csv("data/calculated_parameters/df_ov_params.csv") |> 
...
lme_p12 <- lmer(p12 ~ species * date_fac * year + (1 | sample_id), 
                data = df_ov_params)

lme_p50 <- lmer(p50 ~ species * date_fac * year + (1 | sample_id),
                data = df_ov_params)

lme_p88 <- lmer(p88 ~ species * date_fac  * year + (1 | sample_id),
                data = df_ov_params)

## 1.4.3 emmeans ----

emmeans_p12 <- emmeans(lme_p12, ~ species * date_fac * year, 
                       adjust = "bonferroni", 
                       type = "response")

emmeans_p50 <- emmeans(lme_p50, ~ species * date_fac * year, 
                       adjust = "bonferroni", 
                       type = "response")

emmeans_p88 <- emmeans(lme_p88, ~ species * date_fac * year, 
                       adjust = "bonferroni", 
                       type = "response") 
...
df_emm_p12 <- emmeans_p12 |>
  ...
  arrange(species, date_fac, year) |> 
  mutate(species = dplyr::recode(species, "FASY" = "Fagus sylvatica", "FREX" = "Fraxinus excelsior"),
         group = .group |> f.reletter_cld(),
         date_plot = date,
         group_y = case_when(species == "Fagus sylvatica" ~ 0, species == "Fraxinus excelsior" ~ -6))
...
plot.p12.png <- df_emm_p12 |> 
  ggplot() +
  geom_point(aes(x = date_plot, y = p12, col = species), alpha = plot_alpha,
             position = position_dodge(width = plot_position_offset), show.legend = F) +
  geom_line(aes(x = date_plot, y = emmean, col = species), alpha = 0.2,
            position = position_dodge(width = plot_position_offset), show.legend = F) +
  geom_errorbar(aes(x = date_plot, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
...
ggsave("figures/ov/plot.p12.png", plot.p12.png, width = 8, height = 4, dpi = 150)
...
df_emm_p12 |> 
  group_by(species, year, campaign) |> 
  summarise(mean = mean(p12) |> round(1),
            sd = sd(p12) |> round(1),
            emmean = emmean) |> 
  View()
...
df_emm_p50 <- emmeans_p50 |>
  ...
  mutate(species = dplyr::recode(species, "FASY" = "Fagus sylvatica", "FREX" = "Fraxinus excelsior"),
         group = .group |> f.reletter_cld(),
         date_plot = date,
         group_y = case_when(species == "Fagus sylvatica" ~ 0, species == "Fraxinus excelsior" ~ -6.5))

year(df_emm_p50$date_plot) = 2000

plot.p50.png <- df_emm_p50 |> 
  ggplot() +
  geom_point(aes(x = date_plot, y = p50, col = species), alpha = plot_alpha,
             position = position_dodge(width = plot_position_offset), show.legend = T) +
  geom_line(aes(x = date_plot, y = emmean, col = species), alpha = 0.2,
            position = position_dodge(width = plot_position_offset), show.legend = T) +
  geom_errorbar(aes(x = date_plot, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
  ...
  scale_x_date(limits = c(as.Date("2000-05-15"), as.Date("2000-09-30"))) +
  # labs(y = expression(P[12] ~ "[MPa]"), x = NULL, tag = "a)") +
  labs(y = expression(P[50] ~ "[MPa]"), x = "Date") +  lims(y = c(-6.5, 0)) +
  guides(color = guide_legend(title = "Species")) +
  facet_wrap(~ year) +
  thesis_theme +
  theme(strip.background = element_blank()) +
...
ggsave("figures/ov/plot.p50_datefac.png", plot.p50.png, width = 8, height = 4, dpi = 150)
...
df_emm_p50 |> 
  group_by(species, year, campaign) |> 
  summarise(mean = mean(p50) |> round(1),
            sd = sd(p50) |> round(1),
            emmean = emmean) |> 
  View()
...
df_emm_p88 <- emmeans_p88 |>
  ...
  mutate(species = dplyr::recode(species, "FASY" = "Fagus sylvatica", "FREX" = "Fraxinus excelsior"),
         group = .group |> f.reletter_cld(),
         date_plot = date,
         group_y = case_when(species == "Fagus sylvatica" ~ 0, species == "Fraxinus excelsior" ~ -10))

year(df_emm_p88$date_plot) = 2000

plot.p88.png <- df_emm_p88 |> 
  ggplot() +
  geom_point(aes(x = date_plot, y = p88, col = species), alpha = plot_alpha,
             position = position_dodge(width = plot_position_offset)) +
  geom_line(aes(x = date_plot, y = emmean, col = species), alpha = 0.2,
            position = position_dodge(width = plot_position_offset)) +
  geom_errorbar(aes(x = date_plot, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
...
ggsave("figures/ov/plot.p88.png", plot.p88.png, width = 8, height = 4, dpi = 150)
...
df_emm_p88 |> 
  group_by(species, year, campaign) |> 
  summarise(mean = mean(p88) |> round(1),
            sd = sd(p88) |> round(1),
            emmean = emmean) |> 
  View()
...
plot.p12p50p88_lme.png
ggsave("figures/ov/plot.p12p50p88_lme.png", 
       plot.p12p50p88_lme.png, width = 16, height = 18, dpi = 150, units = "cm")
...
df_emm_p12 |> 
  arrange(year, species, campaign) |> 
  mutate(.group = f.reletter_cld(.group)) |> 
  select(c(species, campaign, year, emmean, SE, df, lower.CL, upper.CL, .group)) |>
  distinct() |> 
  write.table("clipboard", sep = "\t", row.names = F, na = "")

lme_p12 |> broom.mixed::tidy() |> 
  write.table("clipboard", sep = "\t", row.names = F, na = "")
...
lme_t5 <- lmer(estimate ~ species * date_fac + (1 | sample_id), 
               data = df_tsm_params |> filter(trait == "T5"))

lme_t50 <- lmer(estimate ~ species * date_fac + (1 | sample_id), 
                data = df_tsm_params |> filter(trait == "T50"))

lme_t95 <- lmer(estimate ~ species * date_fac + (1 | sample_id),
                data = df_tsm_params |> filter(trait == "T95"))


## 1.5.3 emmeans ----------------------------------------------------------------

emmeans_t5 <- emmeans(lme_t5, ~ species * date_fac, 
                      adjust = "mvt", 
                      type = "response")

emmeans_t50 <- emmeans(lme_t50, ~ species * date_fac, 
                       adjust = "mvt", 
                       type = "response")

emmeans_t95 <- emmeans(lme_t95, ~ species * date_fac,
                       adjust = "mvt", 
                       type = "response")
...
df_emm_tsm <- df_emm_tsm |> 
  ...
  mutate(species = dplyr::recode(species, "FASY" = "Fagus sylvatica", "FREX" = "Fraxinus excelsior"),
         ...
         group_y = case_when(species == "Fagus sylvatica" ~ 65, species == "Fraxinus excelsior" ~ 30),
         plot_number = case_when(trait == "T5" ~ "a)", trait == "T50" ~ "b)", trait == "T95" ~ "c)"),
         trait = case_when(trait == "T5" ~ "T[5]", trait == "T50" ~ "T[50]", trait == "T95" ~ "T[95]"),)    
year(df_emm_tsm$date_plot) = 2000

plot.tsm.png <- df_emm_tsm |> 
  ggplot() +
  geom_point(aes(x = date_plot, y = estimate, col = species), alpha = 0.5,
             position = position_dodge(width = plot_position_offset)) +
  geom_line(aes(x = date_plot, y = emmean, col = species), alpha = 0.2,
            position = position_dodge(width = plot_position_offset)) +
  geom_errorbar(aes(x = date_plot, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
  ...
  guides(color = guide_legend(title = "Species")) +
  scale_color_oi(order = c(6, 2)) +
  facet_grid(cols = vars(trait), scales = "free_x", space = "free_x", 
             labeller = label_parsed) +  # Ensure columns are faceted by hsm_type
  thesis_theme +
  theme(strip.background = element_blank(),
...
ggsave("figures/tsm/plot.tsm_datefac.png", plot.tsm.png, width = 10, height = 4, dpi = 150)


plot.t5.png <- df_emm_t5 |> 
  ggplot() +
  geom_point(aes(x = date_plot, y = estimate, col = species), alpha = plot_alpha,
             position = position_dodge(width = plot_position_offset)) +
  geom_line(aes(x = date_plot, y = emmean, col = species), alpha = 1,
            position = position_dodge(width = plot_position_offset)) +
  geom_errorbar(aes(x = date_plot, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
                position = position_dodge(width = plot_position_offset)) +
  geom_text(aes(x = date_plot, y = group_y, label = .group, col = species), show.legend = F) +
  scale_color_manual(values = c("Fagus sylvatica" = "#410866", "Fraxinus excelsior" = "#faad22")) +
  scale_x_date(limits = c(as.Date("2000-05-15"), as.Date("2000-09-30"))) +
...
df_emm_t50 <- emmeans_t50 |>
  ...
  mutate(species = dplyr::recode(species, "beech" = "Fagus sylvatica", "ash" = "Fraxinus excelsior"),
         date_plot = as.Date(date),
         group_y = case_when(species == "Fagus sylvatica" ~ 60, species == "Fraxinus excelsior" ~ 30))
year(df_emm_t50$date_plot) = 2000
...
ggsave("figures/tsm/plot.t50.png", plot.t50.png, width = 6, height = 4, dpi = 150)
...
ggsave("figures/tsm/plot.t95.png", plot.t95.png, width = 6, height = 4, dpi = 150)


df_emm_tsm |> 
  group_by(trait, species, year, campaign) |> 
  summarise(mean = mean(estimate) |> round(1),
...
df_emm_t5 |> 
  ...
  mutate(.group = f.reletter_cld(.group)) |> 
  dplyr::select(c(species, campaign, emmean, SE, df, lower.CL, upper.CL, .group)) |>
  distinct() |> 
  write.table("clipboard", sep = "\t", row.names = F, na = "")

lme_t5 |> broom.mixed::tidy() |> 
  write.table("clipboard", sep = "\t", row.names = F, na = "")
...
df_hsm <- df_hsm |> 
  mutate(hsm_tlp = psi_midday_mpa - psi_tlp,
         hsm_p12 = psi_midday_mpa - p12,
         hsm_p50 = psi_midday_mpa - p50,
         hsm_p88 = psi_midday_mpa - p88,
         date_plot = date) |> 
  pivot_longer(cols = c(hsm_tlp, hsm_p50, hsm_p12, hsm_p88),
               names_to = "hsm_type", values_to = "hsm_mpa")
year(df_hsm$date_plot) = 2000


## 1.6.2 lme -----------------------------------------------------------------

lme_hsm_tlp <- lmer(hsm_mpa ~ species * campaign * year + (1 | sample_id), 
                    data = df_hsm |> filter(hsm_type == "hsm_tlp"))

lme_hsm_p12 <- lmer(hsm_mpa ~ species * campaign * year + (1 | sample_id),
                    data = df_hsm |> filter(hsm_type == "hsm_p12"))

lme_hsm_p50 <- lmer(hsm_mpa ~ species * campaign * year + (1 | sample_id),
                    data = df_hsm |> filter(hsm_type == "hsm_p50"))

lme_hsm_p88 <- lmer(hsm_mpa ~ species * campaign * year + (1 | sample_id),
...
emmeans_hsm_tlp <- emmeans(lme_hsm_tlp, ~ species * campaign * year, 
                           adjust = "bonferroni", 
                           type = "response")

emmeans_hsm_p12 <- emmeans(lme_hsm_p12, ~ species * campaign * year, 
                           adjust = "bonferroni", 
                           type = "response")

emmeans_hsm_p50 <- emmeans(lme_hsm_p50, ~ species * campaign * year,
                           adjust = "bonferroni", 
                           type = "response")

emmeans_hsm_p88 <- emmeans(lme_hsm_p88, ~ species * campaign * year,
                           adjust = "bonferroni", 
                           type = "response")
...
df_emm_hsm <- bind_rows(df_emm_hsm_tlp, df_emm_hsm_p12, df_emm_hsm_p50, df_emm_hsm_p88)

df_hsm_emm <- left_join(df_hsm, df_emm_hsm, by = c("species", "campaign", "year", "hsm_type")) 
...
df_hsm_emm <- df_hsm_emm |> 
  mutate(
    species = dplyr::recode(species, "FASY" = "Fagus sylvatica", "FREX" = "Fraxinus excelsior"),
    hsm_type = case_when(
    ...
    group_y = case_when(species == "Fagus sylvatica" ~ 6.5, species == "Fraxinus excelsior" ~ -1.5),
    plot_number = case_when(hsm_type == "P[12]" & year  == 2023 ~ "c)",
...
plot.hsm.png <- df_hsm_emm |> 
  ggplot() +
  geom_point(aes(x = date_plot, y = hsm_mpa, col = species),
             position = position_dodge(width = 5), size = 1.5, alpha = plot_alpha) +
  geom_line(aes(x = date_plot, y = emmean, col = species),
            position = position_dodge(width = 5), alpha = plot_alpha, show.legend = FALSE) +
  geom_errorbar(aes(x = date_plot, ymin = lower.CL, ymax = upper.CL, color = species),
                position = position_dodge(width = 5), width = 3, alpha = 0.4, show.legend = FALSE) +
  geom_text(aes(x = date_plot, y = group_y, label = .group, col = species), show.legend = F) +
  geom_text(aes(x = as.Date("2000-06-01"), y = 6.5, label = plot_number), show.legend = F) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(y = "Hydraulic safety margin [MPa]", x = "Date") +
  guides(color = guide_legend(title = "Species")) +
  scale_y_continuous(limits = c(-1.5, 6.5), breaks = seq(-1, 6, 1)) +
  scale_color_oi(order = c(6, 2)) +
  facet_grid(cols = vars(year), rows = vars(hsm_type), scales = "free_x", space = "free_x", 
             labeller = label_parsed) +  # Ensure columns are faceted by hsm_type
  thesis_theme +
  theme(legend.text = element_text(face = "italic"))
plot.hsm.png

ggsave("figures/hsm/plot.hsm.png", plot.hsm.png, width = 8, height = 10, dpi = 150)

df_hsm_emm |> 
  group_by(year, species, campaign, hsm_type) |>
  summarise(mean = mean(hsm_mpa, na.rm = TRUE) |> round(1),
            sd = sd(hsm_mpa, na.rm = TRUE) |> round(1)) |> 
  View()

# 1.7. Sugars ------------------------------------------------------------------

df_sugars_summ <- read.csv("data/calculated_parameters/df_sugars_2023_2024.csv") |> 
  mutate(date = as.Date(date),
         species = as.factor(species),
         sample_id = as.factor(sample_id),
         campaign = as.factor(campaign),
...
df_sugars_nest <- df_sugars_nest |>
  mutate(lme = map(
    data,
    ~ glmmTMB::glmmTMB(sugar_conc ~ species * date_fac + (1 | sample_id),
...
df_sugars_nest <- df_sugars_nest |>
  mutate(emmeans = map(
    lme,
    ~ emmeans::emmeans(.x, ~ species * date_fac,
                       lmer.df = "asymptotic",
                       adjust = "mvt", 
                       type = "response")
  )) 
...
plot.glucose <- df_sugars_emm |>
  ...
  ggplot() +
  geom_point(aes(x = date_plot, y = sugar_conc, col = species), alpha = 1,
             position = position_dodge(width = plot_position_offset), show.legend = FALSE) +
  geom_line(aes(x = date_plot, y = response, group = sample_id, col = species),
            position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = FALSE) +
  # geom_errorbar(aes(x = date_plot, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
  geom_errorbar(aes(x = date_plot, ymin = asymp.LCL, ymax = asymp.UCL, color = species), width = 3,
                  position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = FALSE) +
  geom_text(aes(x = date_plot, y = group_y, label = group, col = species), show.legend = FALSE) +  
  # scale_y_continuous(limits = c(0, 70), breaks = seq(0,70,10)) +
  scale_x_date(limits = c(as.Date("2000-05-15"), as.Date("2000-09-30"))) +
  scale_color_oi(order = c(6, 2)) +
  guides(color = guide_legend(title = "Species")) +
  labs(y = expression("Glucose [mg" ~ g^{-1} * "]"), x = "Date", tag = "c)") +
  facet_wrap(~ year) +
  thesis_theme 
...
plot.starch <- df_sugars_emm |>
  ...
  scale_color_oi(order = c(6, 2)) +
  guides(color = guide_legend(title = "Species")) +
  labs(y = expression("Starch [mg" ~ g^{-1} * "]"), x = "Date", tag = "d)") +
  facet_wrap(~ year) +
  thesis_theme
...
plot.sucrose <- df_sugars_emm |>
  ...
  scale_color_oi(order = c(6, 2)) +
  guides(color = guide_legend(title = "Species")) +
  labs(y = expression("Sucrose [mg" ~ g^{-1} * "]"), x = "Date", tag = "e)") +
  facet_wrap(~ year) +
  thesis_theme 
...
plot.fructose <- df_sugars_emm |>
  ...
  scale_color_oi(order = c(6, 2)) +
  guides(color = guide_legend(title = "Species")) +
  labs(y = expression("Fructose [mg" ~ g^{-1} * "]"), x = "Date", tag = "b)") + 
  facet_wrap(~ year) +
  thesis_theme 
...
plot.total_sugars <- df_sugars_emm |>
  ...
  scale_color_oi(order = c(6, 2)) +
  guides(color = guide_legend(title = "Species")) +
  labs(y = expression("Total sugars [mg" ~ g^{-1} * "]"), x = "Date", tag = "a)") +
  facet_wrap(~ year) +
  thesis_theme 
...
plot.starch_sugar_ratio <- df_sugars_emm |>
  ...
  scale_color_oi(order = c(6, 2)) +
  guides(color = guide_legend(title = "Species")) +
  labs(y = expression("Starch to sugar ratio [mg" ~ mg^{-1} * "]"), x = "Date", tag = "f)") +
  facet_wrap(~ year) +
  thesis_theme 
...
df_sugars_emm |>
  ...
  group_by(species, year, campaign) |>
  summarise(mean = mean(sugar_conc) |> round(1),
...
plots.all_nsc.png <- plot.total_sugars + extra_theme + plot_spacer() + plot.fructose + extra_theme +
  # plot_spacer() + plot_spacer() + plot_spacer() +
  plot.glucose + extra_theme + plot_spacer() + plot.starch + extra_theme +
  # plot_spacer() + plot_spacer() + plot_spacer() +
  plot.sucrose + theme(
    plot.tag.position = c(0.03, 0.95),
    legend.text = element_text(size = 8, face = "italic"),
...
plots.all_nsc.png
ggsave("figures/sugars/all_nsc_both_years.png", plots.all_nsc.png, width = 30, height = 20, dpi = 150, unit = "cm")

# 1.8. Nutrients ---------------------------------------------------------------

df_nutrients_summ <- read.csv("data/calculated_parameters/df_nutrients_2023_2024.csv") |> 
  mutate(date = as.Date(date),
         species = as.factor(species),
...
df_nutrients_nest <- df_nutrients_nest |>
  mutate(lme = map(
    data,
    ~ nlme::lme(nutrient_conc ~ date * species,
...
df_nutrients_nest <- df_nutrients_nest |>
  mutate(emmeans = map(
    lme,
    ~ emmeans::emmeans(.x, ~ species * campaign,
                       adjust = "mvt", 
                       type = "response")
  )) 
...
df_nutrients_emm <- df_nutrients_emm |> 
  ...
  mutate(min = min(nutrient_conc),
         ...
         species = recode(species, "FASY" = "Fagus sylvatica", 
...
plot.c_con_percent <- df_nutrients_emm |> 
  ...
  arrange(species, campaign) |> 
  mutate(group_y = case_when(species == "Fagus sylvatica" ~ 52,
                             species == "Fraxinus excelsior" ~ 42),
         group = .group |> f.reletter_cld()) |> 
  ggplot() +
  geom_point(aes(x = date, y = nutrient_conc, col = species), alpha = 1,
             position = position_dodge(width = plot_position_offset), show.legend = FALSE) +
  geom_line(aes(x = date, y = emmean, group = sample_id, col = species),
            position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = FALSE) +
  geom_errorbar(aes(x = date, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
                position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = FALSE) +
  geom_text(aes(x = date, y = group_y, label = group, col = species), show.legend = FALSE) +
  annotate("text", x = as.Date("2023-06-01"), y = 52, label = "a)", size = 5, vjust = 0.5, hjust = 0) + 
...
ggsave("figures/nutrients/c_con_percent.png", plot.c_con_percent, width = 8, height = 6, dpi = 150, unit = "cm")

### Nitrogen ----
plot.n_con_percent <- df_nutrients_emm |> 
  filter(nutrient_name == "n_con_percent") |> 
  arrange(species, campaign) |> 
  mutate(group_y = case_when(species == "Fagus sylvatica" ~ 4,
                             species == "Fraxinus excelsior" ~ 0),
         group = .group |> f.reletter_cld()) |> 
  ggplot() +
  geom_point(aes(x = date, y = nutrient_conc, col = species), alpha = 1,
  ...
  geom_errorbar(aes(x = date, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
                position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = FALSE) +
  geom_text(aes(x = date, y = group_y, label = group, col = species), show.legend = FALSE) +
  annotate("text", x = as.Date("2023-06-01"), y = 4, label = "b)", size = 5, vjust = 0.5, hjust = 0) + 
  scale_x_date(limits = c(as.Date("2023-06-01"), as.Date("2023-09-30"))) +
  scale_color_oi(order = c(6, 2)) +
  guides(color = guide_legend(title = "Species")) +
...
ggsave("figures/nutrients/n_con_percent.png", plot.n_con_percent, width = 8, height = 6, dpi = 150, unit = "cm")
...
plot.p_con_mg_kg <- df_nutrients_emm |> 
  ...
  arrange(species, campaign) |> 
  mutate(group_y = case_when(species == "Fagus sylvatica" ~ 2000,
  ...
  geom_errorbar(aes(x = date, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
                position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = FALSE) +
  geom_text(aes(x = date, y = group_y, label = group, col = species), show.legend = FALSE) +
  annotate("text", x = as.Date("2023-06-01"), y = 2000, label = "c)", size = 5, vjust = 0.5, hjust = 0) + 
  scale_x_date(limits = c(as.Date("2023-06-01"), as.Date("2023-09-30"))) +
  scale_color_oi(order = c(6, 2)) +
  guides(color = guide_legend(title = "Species")) +
...
ggsave("figures/nutrients/p_con_mg_kg.png", plot.p_con_mg_kg, width = 8, height = 6, dpi = 150, unit = "cm")
...
plot.s_con_mg_kg <- df_nutrients_emm |> 
  ...
  arrange(species, campaign) |> 
  mutate(group_y = case_when(species == "Fagus sylvatica" ~ 3000,
                             species == "Fraxinus excelsior" ~ 0),
         group = .group |> f.reletter_cld()) |> 
  ggplot() +
  geom_point(aes(x = date, y = nutrient_conc, col = species), alpha = 1,
  ...
  geom_errorbar(aes(x = date, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
                position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = FALSE) +
  geom_text(aes(x = date, y = group_y, label = group, col = species), show.legend = FALSE) +
  annotate("text", x = as.Date("2023-06-01"), y = 3000, label = "d)", size = 5, vjust = 0.5, hjust = 0) + 
  scale_x_date(limits = c(as.Date("2023-06-01"), as.Date("2023-09-30"))) +
  scale_color_oi(order = c(6, 2)) +
  guides(color = guide_legend(title = "Species")) +
...
ggsave("figures/nutrients/s_con_mg_kg.png", plot.s_con_mg_kg,  width = 8, height = 6, dpi = 150, unit = "cm")

### Calcium ----
plot.ca_con_mg_kg <- df_nutrients_emm |> 
  filter(nutrient_name == "ca_con_mg_kg") |> 
  arrange(species, campaign) |> 
  mutate(group_y = case_when(species == "Fagus sylvatica" ~ 25000,
                             species == "Fraxinus excelsior" ~ 0),
         group = .group |> f.reletter_cld()) |> 
  ggplot() +
  geom_point(aes(x = date, y = nutrient_conc, col = species), alpha = 1,
  ...
  geom_errorbar(aes(x = date, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
                position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = FALSE) +
  geom_text(aes(x = date, y = group_y, label = group, col = species), show.legend = FALSE) +
  annotate("text", x = as.Date("2023-06-01"), y = 25000, label = "g)", size = 5, vjust = 0.5, hjust = 0) + 
  scale_x_date(limits = c(as.Date("2023-06-01"), as.Date("2023-09-30"))) +
  scale_color_oi(order = c(6, 2)) +
  guides(color = guide_legend(title = "Species")) +
...
ggsave("figures/nutrients/ca_con_mg_kg.png", plot.ca_con_mg_kg, width = 8, height = 6, dpi = 150, unit = "cm")

### Potassium ----
plot.k_con_mg_kg <- df_nutrients_emm |> 
  filter(nutrient_name == "k_con_mg_kg") |> 
  arrange(species, campaign) |> 
  mutate(group_y = case_when(species == "Fagus sylvatica" ~ 15000,
                             species == "Fraxinus excelsior" ~ 0),
         group = .group |> f.reletter_cld()) |> 
  ggplot() +
  geom_point(aes(x = date, y = nutrient_conc, col = species), alpha = 1,
  ...
  geom_errorbar(aes(x = date, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
                position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = FALSE) +
  geom_text(aes(x = date, y = group_y, label = group, col = species), show.legend = FALSE) +
  annotate("text", x = as.Date("2023-06-01"), y = 15000, label = "f)", size = 5, vjust = 0.5, hjust = 0) + 
  scale_x_date(limits = c(as.Date("2023-06-01"), as.Date("2023-09-30"))) +
  scale_color_oi(order = c(6, 2)) +
...
ggsave("figures/nutrients/k_con_mg_kg.png", plot.k_con_mg_kg, width = 8, height = 8.5, dpi = 150, unit = "cm")
...
plot.mg_con_mg_kg <- df_nutrients_emm |> 
  ...
  arrange(species, campaign) |> 
  mutate(group_y = case_when(species == "Fagus sylvatica" ~ 3100,
                             species == "Fraxinus excelsior" ~ 0),
         group = .group |> f.reletter_cld()) |> 
  ggplot() +
  geom_point(aes(x = date, y = nutrient_conc, col = species), alpha = 1,
  ...
  geom_errorbar(aes(x = date, ymin = lower.CL, ymax = upper.CL, color = species), width = 3,
                position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = FALSE) +
  geom_text(aes(x = date, y = group_y, label = group, col = species), show.legend = FALSE) +
  annotate("text", x = as.Date("2023-06-01"), y = 3100, label = "e)", size = 5, vjust = 0.5, hjust = 0) + 
  scale_x_date(limits = c(as.Date("2023-06-01"), as.Date("2023-09-30"))) +
  scale_color_oi(order = c(6, 2)) +
  guides(color = guide_legend(title = "Species")) +
...
ggsave("figures/nutrients/mg_con_mg_kg.png", plot.mg_con_mg_kg, width = 8, height = 6, dpi = 150, unit = "cm")
...
lm_psi_md <- lm(
  trait_val ~ leaf_age_d + max_ta_50 + sum_p_45 + mean_rew_1 + 
    mean_ta_60 + sum_p_et_60 + sum_et_35 + mean_et_pet_1 + mean_vpd_35,
...
df_psi_md_meteo <- df_psi_md_meteo |> 
  mutate(species = dplyr::recode(species, "FREX" = "Fraxinus excelsior", "FASY" = "Fagus sylvatica"),
...
plot.psi_md_meteo.png <- df_psi_md_meteo |> 
  ggplot() +
  geom_point(aes(x = leaf_age_d, y = trait_val, col = species, shape = year), show.legend = F) +
  geom_line(aes(x = leaf_age_d, y = pred, col = species, group = interaction(species, year, sample_id), linetype = year), 
            alpha = 0.5, linewidth = 0.5, show.legend = F) +
...
plot.psi_midday_comb.png <- (plot.psi_md_meteo.png + plot_spacer() + plot.psi_md_coefs.png) + 
  plot_layout(widths = c(11, -0.9, 5)) 
plot.psi_midday_comb.png
# Save the updated plot
ggsave("figures/psi_midday/plot.psi_midday_comb.png", 
       plot.psi_midday_comb.png, width = 16, height = 8, dpi = 150, units = "cm")

df_psi_md_meteo |> 
  ggplot() +
  geom_point(aes(x = sum_et_35, y = trait_val, col = species, shape = year))

mgcv::gamm(trait_val ~ mean_rew_1 + sum_et_35, 
           random = 1 | species/sample_id,
           data = df_psi_md_meteo) |> summary()

df_psi_md_meteo$preds_test <- mgcv::gam(trait_val ~ sum_p_et_60 + mean_rew_1 + max_ta_50 + sum_et_35, 
                                        data = df_psi_md_meteo) |> 
  predict()

ggplot(df_psi_md_meteo) +
  geom_point(aes(x = leaf_age_d, y = trait_val, col = species, shape = year)) +
  geom_line(aes(x = leaf_age_d, y = preds_test, col = species, group = interaction(species, year, sample_id), linetype = year), alpha = 0.5,
            linewidth = 0.5)
...
lm_psi_ft <- lm(
  trait_val ~ leaf_age_d + mean_vpd_45 + mean_rh_45 + max_ta_35 + mean_rew_60 + sum_pet_60 + mean_ta_40 + sum_ppfd_60 + sum_p_et_45 + sum_et_50 + mean_et_pet_20 + sum_p_45,
...
plot.psi_ft_meteo.png <- df_psi_ft_meteo |> 
  ggplot() +
  geom_point(aes(x = leaf_age_d, y = trait_val, col = species, shape = year), show.legend = F) +
  geom_line(aes(x = leaf_age_d, y = pred, col = species, group = interaction(species, year, sample_id), linetype = year), alpha = 0.5,
            linewidth = 0.5, show.legend = F) +
  scale_x_continuous(limits = c(0, 180), breaks = seq(0, 180, 20)) +
  scale_y_continuous(limits = c(-4, 0)) + scale_shape_manual(values = c("2023" = 16, "2024" = 2)) +
  scale_shape_manual(values = c("2023" = 16, "2024" = 1)) +
  scale_color_oi(order = c(6, 2)) +
  labs(y = "Osm. potential at full turgor [MPa]", x = "Leaf age [days]", tag = "c)") +
  guides(color = guide_legend(title = "Species", ncol = 2),
         shape = guide_legend(title = "Year", ncol = 2),
         linetype = guide_legend(title = "Year", ncol = 2)) +
  # annotate("text", x = 0, y = 0, label = "a)") +
...
plot.psi_ft_comb.png <- (plot.psi_ft_meteo.png + plot_spacer() + plot.psi_ft_coefs.png) + 
  plot_layout(widths = c(10, 0, 6)) 
plot.psi_ft_comb.png

ggsave("figures/psi_ft/plot.psi_ft_comb.png", 
       plot.psi_ft_comb.png, width = 16, height = 8, dpi = 150, units = "cm")
...
lm_psi_tlp <- lm(
  trait_val ~ leaf_age_d + mean_vpd_35 + mean_rew_60 + mean_rh_45 + sum_pet_60 + max_ta_35 + mean_ta_35 + sum_ppfd_60 + sum_p_et_45 + sum_et_1 + sum_p_45 + mean_et_pet_60,
...
plot.psi_tlp_meteo.png <- df_psi_tlp_meteo |> 
  ggplot() +
  geom_point(aes(x = leaf_age_d, y = trait_val, col = species, shape = year)) +
  geom_line(aes(x = leaf_age_d, y = pred, col = species, group = interaction(species,year), 
                linetype = year), alpha = 0.5, linewidth = 0.5) +
  scale_x_continuous(limits = c(0, 180), breaks = seq(0, 180, 20)) +
  scale_y_continuous(limits = c(-4, 0)) +
  scale_shape_manual(values = c("2023" = 16, "2024" = 1)) +
  scale_color_oi(order = c(6, 2)) +
  labs(y = "Osm. potential at turgor loss [MPa]", x = "Leaf age [days]", tag = "e)") +
  guides(color = guide_legend(title = "Species", ncol = 2),
         shape = guide_legend(title = "Year", ncol = 2),
         linetype = guide_legend(title = "Year", ncol = 2)) +
  # annotate("text", x = 0, y = 0, label = "a)") +
...
plot.psi_tlp_comb.png <- (plot.psi_tlp_meteo.png + plot_spacer() + plot.psi_tlp_coefs.png) + 
  plot_layout(widths = c(11, -0.9, 5)) 
plot.psi_tlp_comb.png

ggsave("figures/psi_tlp/plot.psi_tlp_comb.png", 
       plot.psi_tlp_comb.png, width = 16, height = 8, dpi = 150, units = "cm")
...
plot.psi_md_ft_tlp.png <- 
  plot.psi_md_meteo.png + plot_spacer() + plot.psi_md_coefs.png +
  plot.psi_ft_meteo.png + plot_spacer() + plot.psi_ft_coefs.png +
  plot.psi_tlp_meteo.png + plot_spacer() + plot.psi_tlp_coefs.png +
  plot_layout(widths = c(11, -0.9, 5), heights = c(8,8,8))
plot.psi_md_ft_tlp.png

ggsave("figures/plot.psi_md_ft_tlp.png", plot.psi_md_ft_tlp.png, width = 16, height = 22, units = "cm")
...
df_elast_meteo_scaled <- df_elast_meteo |> 
  select(c(trait, date, year, species, sample_id, campaign, trait_val, doy)) |> 
  cbind(df_elast_meteo |> 
          select(-c(trait, date, year, species, sample_id, campaign, trait_val, doy)) |>
          scale())

f.make_formula_char(df_elast_meteo) |> writeClipboard()

lm_elast <- lm(trait_val ~ leaf_age_d + mean_ta_60 + max_ta_55 + sum_pet_20 + sum_et_60 + sum_ppfd_5 + mean_rh_20 + mean_rew_60 + sum_p_55 + mean_vpd_15,
...
glme_elast <- glmmTMB::glmmTMB(
  trait_val ~ leaf_age_d + max_ta_55 + sum_pet_20 + 
    sum_et_60 + (1 | species),
  data = df_elast_meteo_scaled,
  family = Gamma(link = "log")
)
...
df_elast_meteo <- df_elast_meteo |> 
  mutate(pred = predict(glme_elast, type = "response"),
         date_plot = date,
         year = as.factor(year))
...
plot.elast_meteo.png <- df_elast_meteo |> 
  ...
  ggplot() +
  geom_point(aes(x = leaf_age_d, y = trait_val, col = species, shape = year), show.legend = F) +
  geom_line(aes(x = leaf_age_d, y = pred, col = species, group = interaction(species,year, sample_id), 
                linetype = year), alpha = 0.2, linewidth = 0.5, show.legend = F) +
  scale_x_continuous(limits = c(0, 180), breaks = seq(0, 180, 20)) +
  scale_y_continuous(limits = c(0, 40)) +
  scale_shape_manual(values = c("2023" = 16, "2024" = 1)) +
  scale_color_oi(order = c(6, 2)) +
  labs(y = "Bulk modulus of elasticity [MPa]", x = "Leaf age [days]", tag = "a)") +
  guides(color = guide_legend(title = "Species", ncol = 2),
         shape = guide_legend(title = "Year", ncol = 2),
         linetype = guide_legend(title = "Year", ncol = 2)) +
...
glme_elast |> ranef() |> 
  as.data.frame() |> dplyr::select(term, grp, condval) |>
...
lm_rwc_tlp <- lm(
  trait_val ~ leaf_age_d + max_ta_60 + sum_et_60 + mean_ta_60 + 
    sum_pet_20 + sum_ppfd_5 + mean_et_pet_50 + mean_rew_30 + sum_p_et_50 + mean_rh_20,
...
lme_rwc_tlp <- nlme::lme(
  trait_val ~ leaf_age_d + sum_et_60 + 
    sum_pet_20 + mean_rew_30,
  random = ~ leaf_age_d | species/sample_id,
...
plot.rwc_tlp_meteo.png <- df_rwc_tlp_meteo |> 
  ...
  geom_point(aes(x = leaf_age_d, y = trait_val, col = species, shape = year), show.legend = F) +
  geom_line(aes(x = leaf_age_d, y = pred, col = species, group = interaction(species,year, sample_id), 
                linetype = year), alpha = 0.5, linewidth = 0.5, show.legend = F) +
  scale_x_continuous(limits = c(0, 180), breaks = seq(0, 180, 20)) +
...
lm_gmin <- lm(
  trait_val ~ leaf_age_d + mean_et_pet_60 + max_ta_55 + sum_pet_30 + sum_ppfd_30 + 
    mean_ta_60 + mean_rh_35 + mean_vpd_15 + sum_p_2 + sum_et_10 + sum_p_et_2,
...
plot.gmin_coefs.png <-
  sjPlot::plot_model(glme_gmin, type = "est",
                     show.values = TRUE, show.p = T, value.offset = 0.3, line.size = 0.5, dot.size = 1.3) +
  geom_hline(aes(yintercept = 0), alpha= 0.2)+
  scale_x_discrete(labels = new_labels) +
  labs(y = "Coefficient estimates", title = NULL, tag = "f)") +
  ...
  theme(axis.text = element_text(size = 8),
        text = element_text(size = 6))
plot.gmin_coefs.png
ggsave("figures/gmin/plot.gmin_coefs.png",
       plot.gmin_coefs.png, width = 6, height = 8, dpi = 150, units = "cm")
...
plot.gmin_meteo.png <- df_gmin_meteo |> 
  ...
  scale_shape_manual(values = c("2023" = 16, "2024" = 1)) +
  scale_color_oi(order = c(6, 2)) +
  labs(y = expression(g[min] ~ "["*mmol~m^{-2}~s^{-1}*"]"), x = "Leaf age [days]", tag = "e)") +
  guides(color = guide_legend(title = "Species", ncol = 2),
         shape = guide_legend(title = "Year", ncol = 2),
         linetype = guide_legend(title = "Year", ncol = 2)) +
...
plot.eps_rwc_gmin.png <- 
  plot.elast_meteo.png + plot_spacer() + plot.elast_coefs.png +
  plot.rwc_tlp_meteo.png + plot_spacer() + plot.rwc_tlp_coefs.png +
  plot.gmin_meteo.png + plot_spacer() + plot.gmin_coefs.png +
  plot_layout(widths = c(11, -0.9, 5), heights = c(8,8,8))
plot.eps_rwc_gmin.png

ggsave("figures/plot.eps_rwc_gmin.png", plot.eps_rwc_gmin.png, width = 16, height = 22, units = "cm")
...
lm_p12 <- lm(
  trait_val ~ leaf_age_d + sum_p_2 + max_ta_55 + sum_p_et_5 + mean_et_pet_25 + mean_ta_60,
...
ggsave("figures/p12/plot.p12_comb.png", 
       plot.p12_comb.png, width = 16, height = 8, dpi = 150, units = "cm")
...
lm_p50 <- lm(
  trait_val ~ leaf_age_d + sum_p_2 + mean_et_pet_25 + sum_pet_30 + mean_rh_40 + max_ta_55 + sum_ppfd_40 + mean_ta_60 + sum_p_et_2,
...
ggsave("figures/p50/plot.p50_comb.png", 
       plot.p50_comb.png, width = 16, height = 8, dpi = 150, units = "cm")
...
lm_p88 <- lm(
  trait_val ~ leaf_age_d + sum_p_2 + mean_rh_55 + mean_et_pet_25 + sum_p_et_2 + sum_pet_30 + max_ta_55 + mean_vpd_35,
...
ggsave("figures/p88/plot.p88_comb.png", 
       plot.p88_comb.png, width = 16, height = 8, dpi = 150, units = "cm")
...
plot.p12p50p88_comb.png <- 
  plot.p12_meteo.png + plot_spacer() + plot.p12_coefs.png +
  plot.p50_meteo.png + plot_spacer() + plot.p50_coefs.png +
  plot.p88_meteo.png + plot_spacer() + plot.p88_coefs.png + 
  plot_layout(widths = c(11, -0.9, 5), heights = c(8,8,8))

plot.p12p50p88_comb.png
ggsave("figures/plot.p12p50p88_comb.png", 
       plot.p12p50p88_comb.png, width = 16, height = 24, dpi = 150, units = "cm")


# 3. NSC models -----------------------------------------------------------

df_psi_md_NSC <- df_psi_midday %>% 
  left_join(df_sugars_emm %>% 
              select(date, species, sample_id, sugar_name, sugar_conc) %>% 
              na.omit() %>% 
              mutate(date = case_when(date == "2023-06-23" ~ "2023-06-13" %>% as.Date(),
...
df_psi_md_NSC %>% filter(sugar_name == "total_sugars_mg_g") %>% 
  mutate(date_plot = date %>% str_replace("2023|2024", "2000") %>% as.Date()) %>% 
  ggplot() +
  geom_point(aes(x = date_plot, y = psi_midday_mpa, col = species)) +
  geom_line(aes(x = date_plot, y = predict(lme_psimd_totalsugars_2), col = species, 
...
df_hydr_traits_nest <- df_hydr_traits_nest |> 
  mutate(
        mod.leaf_age = map(data, ~ try(nlme::lme(value ~ leaf_age_d , 
                                         random = ~ 1 | species/sample_id,
                                         data = .x))))
...
df_hydr_traits_nest <- df_hydr_traits_nest |> 
  mutate(
    mod.total_sugars = map(data, ~ try(nlme::lme(value ~ total_sugars_mg_g * glucose_mg_g, 
                                             random = ~ 1 | species,
                                             correlation = corAR1(form = ~ leaf_age_d|species/sample_id),
                                             data = .x))))

df_hydr_traits_nest <- df_hydr_traits_nest |> 
  mutate(data = map2(data, mod.total_sugars, ~ .x |> 
...
df_hydr_traits_nest$data[[11]] |> 
  ggplot() +
  geom_point(aes(x = date_plot, y = value, col = species, group = species)) +
  geom_line(aes(x = date_plot, y = pred_total_sugars,
                col = species, group = interaction(species, sample_id))) +
  facet_wrap(~year)
...
df_hydr_traits_nest <- df_hydr_traits_nest |> 
  mutate(
    mod.all = map(data, ~ try(nlme::lme(value ~ glucose_mg_g + fructose_mg_g +
                                          sucrose_mg_g + starch_mg_g + glucose_starch_1, 
...
df_hydr_traits_nest$data[[trait_num]] |> 
  ...
  ggplot() +
  geom_point(aes(x = date_plot, y = value, col = species, group = species)) +
  geom_line(aes(x = date_plot, y = pred,
                col = species, group = interaction(species, sample_id))) +
  facet_wrap(~year)
...
colnames(df_hydr_traits_coefs) = c("trait", "intercept",
                                "glucose_mg_g", "fructose_mg_g",
                                "sucrose_mg_g", "starch_mg_g", "glucose/starch")

df_hydr_traits_coefs |> write.table(file = "clipboard", sep = "\t", dec = ".",
                                    row.names = F)
## process data ------------------------------------------------------------
...
df_hydr_traits <- df_pv_params |> ## this first because campaign 1 2024 didn't have psi midday
  left_join(df_psi_midday %>% 
              dplyr::select(-c(doy, date_fac)),
            by = c("date", "species", "sample_id", "campaign", "year")) |>
  left_join(df_gmin_params |>
              filter(rwc_class == "rwc_class_standard") |>
              select(-c(rwc_class, rwc_interval)),
            by = c("date", "species", "sample_id", "campaign", "year")) |>
  left_join(df_ov_params |>
              select(-c(rmse_fitplc:date_plot)),
            by = c("date", "species", "sample_id", "campaign", "year")) |>
  left_join(df_sugars_summ %>% 
              select(date, species, sample_id, sugar_name, sugar_conc) %>% 
              na.omit() %>% 
              mutate(date = case_when(date == "2023-06-23" ~ "2023-06-13" %>% as.Date(),
                                      date == "2023-08-08" ~ "2023-08-09" %>% as.Date(),
...
write.csv(file = "data/calculated_parameters/df_hydr_traits.csv", df_hydr_traits, row.names = F)
...
mod.dregde_psi_tlp_frex <- mod.red_psi_tlp_frex |> MuMIn::dredge(rank = "AICc", m.max = 7)
...
df_psi_tlp_frex_wide |> 
  ...
  geom_line(aes(x = leaf_age_d, y = predict(mod.dregded_psi_tlp_frex), 
...
mod.lme_dredge_psi_tlp_frex <- mod.lme_psi_tlp_frex |> MuMIn::dredge(rank = "AICc", m.max = 7)
...
df_psi_tlp_fasy_wide |> 
  ...
  geom_line(aes(x = leaf_age_d, y = predict(mod.red_psi_tlp_fasy), 
                group = sample_id, col = "reduced with StepAIC")) +
  geom_line(aes(x = leaf_age_d, y = predict(mod.dregded_psi_tlp_fasy), 
                group = sample_id, col = "reduced with Dredge")) +
  facet_wrap(~year) +
  labs(title = "Turgor loss point model with NSC, Nutrients and Microclimate") +
  theme_bw() +
...
mod.lme_dredge_psi_tlp_fasy <- mod.lme_red_psi_tlp_fasy |> MuMIn::dredge(rank = "AICc", m.max = 7)
...
mod.dredge_elast_frex <- mod.red_elast_frex |> MuMIn::dredge(rank = "AICc", m.max = 7)
...
mod.lme_dredge_elast_frex <- mod.lme_elast_frex |> MuMIn::dredge(rank = "AICc", m.max = 7)
...
mod.dredge_elast_fasy <- mod.red_elast_fasy |> MuMIn::dredge(rank = "AICc", m.max = 7)
...
mod.lme_dredge_elast_fasy <- mod.lme_dredged_elast_fasy |> MuMIn::dredge(rank = "AICc", m.max = 7)
...
mod.dredge_gmin_frex <- mod.red_gmin_frex |> MuMIn::dredge(rank = "AICc", m.max = 7)
...
mod.lme_dredge_gmin_frex <- mod.lme_gmin_frex |> MuMIn::dredge(rank = "AICc", m.max = 7)
...
mod.dredge_gmin_fasy <- mod.red_gmin_fasy |> MuMIn::dredge(rank = "AICc", m.max = 7)
...
mod.lme_dredge_gmin_fasy <- mod.lme_gmin_fasy |> MuMIn::dredge(rank = "AICc", m.max = 7)
...
mod.dregde_p50_frex <- mod.red_p50_frex |> MuMIn::dredge(rank = "AICc", m.max = 7)
...
mod.dregde_p50_fasy <- mod.red_p50_fasy |> MuMIn::dredge(rank = "AICc", m.max = 5)
...
mod.lme_dredge_p50_fasy <- mod.lme_p50_fasy |> MuMIn::dredge(rank = "AICc", m.max = 7)
