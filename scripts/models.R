...
lme_psi_midday <- lmer(psi_midday_mpa ~ species * campaign * year + 
                         (1 | sample_id),
                       # family = Gaussian(),
                       data = df_psi_midday)
...
emm_psi_midday <- emmeans(lme_psi_midday, ~ species * campaign * year, 
                          adjust = "bonferroni", 
                          type = "response")
...
df_psi_midday_emmeans <- df_psi_midday |> 
  left_join(df_emm_psi_midday, by = c("species", "campaign", "year")) |> 
...
lme_psi_ft <- lmer(psi_ft ~ species * campaign * year + (1 | sample_id), 
                   data = df_pv_params)
lme_psi_ft |> summary()
lme_psi_ft |> r2()
...
ggplot(df_pv_params) +
  ...
  geom_line(aes(x = date, y = predict(lme_psi_ft), color = species, group = sample_id)) +
  facet_wrap(~ year, scales = "free_x") +
  thesis_theme

lme_psi_tlp <- lmer(psi_tlp ~ species * campaign * year + (1 | sample_id), data = df_pv_params)
lme_psi_tlp |> summary()
lme_psi_tlp |> r2()
...
lme_elast <- lmer(elasticity_tot ~ species * campaign * year + (1 | sample_id), data = df_pv_params)
lme_elast |> summary()
lme_elast |> r2()

ggplot(df_pv_params) +
  geom_point(aes(x = date, y = elasticity_tot, color = species)) +
  geom_line(aes(x = date, y = predict(lme_elast), color = species, group = sample_id)) +
  facet_wrap(~ year, scales = "free_x") +
  thesis_theme

lme_rwc_tlp <- lmer(rwc_tot_tlp ~ species * campaign * year + (1 | sample_id), data = df_pv_params)
lme_rwc_tlp |> summary()
lme_rwc_tlp |> r2()
...
df_sugars_nest <- df_sugars_nest |>
  mutate(lme = map(
    data,
    ~ glmmTMB::glmmTMB(sugar_conc ~ campaign * species +
                         (1 | sample_id),
...
df_sugars_emm <- df_sugars_summ |>
  left_join(
    df_sugars_nest |>
      ...
      unnest(emmgrid),
    by = c("sugar_name", "species", "campaign")
  )
...
plot.glucose <- df_sugars_emm |>
  ...
  ggplot() +
  geom_point(aes(x = date, y = sugar_conc, col = species), alpha = 1,
             position = position_dodge(width = plot_position_offset), show.legend = FALSE) +
  geom_line(aes(x = date, y = response, group = sample_id, col = species),
            position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = FALSE) +
  geom_errorbar(aes(x = date, ymin = asymp.LCL, ymax = asymp.UCL, color = species), width = 3,
                position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = FALSE) +
...
plot.sucrose <- df_sugars_emm |>
  ...
  mutate(species = recode(species, "FASY" = "Fagus sylvatica", "FREX" = "Fraxinus excelsior"),
         group_y = case_when(
           species == "Fagus sylvatica" ~ 72,
           species == "Fraxinus excelsior" ~ -3),
         group = .group |> f.reletter_cld()
  ) |>  
  ggplot() +
  geom_point(aes(x = date, y = sugar_conc, col = species), alpha = 1,
  ...
  geom_errorbar(aes(x = date, ymin = asymp.LCL, ymax = asymp.UCL, color = species), width = 3,
                position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = FALSE) +
  geom_text(aes(x = date, y = group_y, label = group, col = species), show.legend = FALSE) +
  scale_y_continuous(limits = c(-3, 72), breaks = seq(0,70,10)) +
  scale_x_date(limits = c(as.Date("2023-06-01"), as.Date("2023-09-30"))) +
  scale_color_oi(order = c(6, 2)) +
  guides(color = guide_legend(title = "Species")) +
...
plot.fructose <- df_sugars_emm |>
  ...
  geom_errorbar(aes(x = date, ymin = asymp.LCL, ymax = asymp.UCL, color = species), width = 3,
                position = position_dodge(width = plot_position_offset), alpha = 0.2, show.legend = FALSE) +
  geom_text(aes(x = date, y = group_y, label = group, col = species), show.legend = FALSE) +  
  scale_y_continuous(limits = c(0, 42), breaks = seq(0,50,10)) +
  scale_x_date(limits = c(as.Date("2023-06-01"), as.Date("2023-09-30"))) +
  scale_color_oi(order = c(6, 2)) +
  guides(color = guide_legend(title = "Species")) +
...
plots.all_nsc.png <- plot.total_sugars + extra_theme + plot_spacer() + plot.fructose + extra_theme +
  ...
  plot.sucrose + theme(
    ...
    legend.text = element_text(size = 8, face = "italic"),
    legend.justification = c(0.25,0)) +
  plot_layout(widths = c(8, -0.9, 8), heights = c(7, 7, 7))
plots.all_nsc.png
ggsave("figures/sugars/all_nsc.png", plots.all_nsc.png, width = 16, height = 21, dpi = 150, unit = "cm")
...
df_nutrients_nest <- df_nutrients_nest |>
  mutate(lme = map(
    data,
    ~ nlme::lme(nutrient_conc ~ campaign * species,
                random = ~ campaign | sample_id,
                correlation = corCAR1(form = ~ campaign | sample_id),
                data = .x)
  )) 
...
plot.psi_midday_comb.png <- (plot.psi_midday_meteo.png + plot_spacer() + plot.psi_midday_coefs.png) + 
