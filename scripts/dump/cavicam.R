...
plot.ov_cum_emb <- df_wp_area |> 
  ...
  mutate(year = as.factor(year),
         all_id = paste(sample_id, "campaign", campaign),
         elapsed_time_h = elapsed_time_min / 60) |> 
  filter(vessel_order == "all") |> 
  ggplot() +
  geom_point(aes(x = elapsed_time_h, y = perc_area_cav, color = year)) +
  geom_line(aes(x = elapsed_time_h, y = perc_area_cav, color = year)) +
  labs(x = "Elapsed time [hours]", y = "Leaf water potential [MPa]") +
...
## which samples could not be fit with the "mpd" (monotonic decreasing) spline?
...
write.csv(df_ov_params, "data/calculated_parameters/df_ov_params_updated.csv", row.names = F)
...
# try to fix 2024 camp1 FREX data -----------------------------------------

df.frex_camp1 <- df_wp_area |> 
  dplyr::select(c(year, campaign, sample_id, vessel_order, area)) |> 
  unnest(col = c(area)) |>
  mutate(year = as.factor(year),
         all_id = paste(sample_id, "campaign", campaign),
         elapsed_time_h = elapsed_time_min / 60) %>% 
  filter(campaign == 1, year == 2024, sample_id %>% str_detect("FREX"))

df.frex_camp1 %>% 
  filter(vessel_order == "major") |> 
  group_by(all_id) %>% 
  mutate(max_psi = max(psi_pred),
         max_psi_meas = case_when(sample_id == "FREX_05" ~ 43.3,
                                  sample_id == "FREX_06" ~ 28.9,
                                  sample_id == "FREX_07" ~ 38.0,
                                  sample_id == "FREX_08" ~ 24.7,
                                  ),
         psi_scale = (psi_pred - 0) / (max_psi - 0)) %>% 
  # filter(psi_pred <= max_psi_meas) %>% 
  ggplot() +
  geom_point(aes(x = elapsed_time_h, y = perc_area_cav, color = year)) +
  geom_line(aes(x = elapsed_time_h, y = perc_area_cav, color = year)) +
  geom_line(aes(x = elapsed_time_h, y = psi_pred / 100, color = "psi")) +
  scale_y_continuous(name = "PEP", sec.axis = sec_axis(trans=~.*10,name = "Psi_pred")) +
  # geom_line(aes(x = elapsed_time_h, y = psi_scale, color = "psi")) +
  labs(x = "Elapsed time [hours]", y = "Leaf water potential [MPa]") +
  facet_wrap(~ all_id, ncol = 1, scales = "free") +
  theme_bw() +
  theme(axis.text = element_text(size = 5),
        legend.position = "bottom")
