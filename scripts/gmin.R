...
plot.gmin_rwc.png <- df_gmin |> 
  ...
  labs(x = "Relative water content [%]",
       y = expression(g[min] ~ "["*mmol~m^{-2}~s^{-1}*"]")) +
  lims(x = c(100, 0)) +
...
ggsave("figures/gmin/plot.gmin_rwc.png", plot.gmin_rwc.png, width = 16, height = 24, units = "cm")
...
df_gmin_interpolated |> write.csv("data/gmin/gmin_interpolated.csv", row.names = FALSE)
...
df_gmin_interpolated |> 
  ...
  ggplot() +
  geom_line(aes(x = elapsed_time_min, y = leaf_mass_g, group = interaction(sample_id, campaign),
...
df_gmin_slope_nest <- df_gmin_interpolated |>
  mutate(
    ...
    rwc_class_standard = case_when(rwc >= 65 & rwc < 90 ~ "standard", T ~ "other"),
    rwc_class_tlp = case_when(rwc >= 65 & rwc < rwc_tot_tlp ~ "rwc_tlp_90", T ~ "other")
  ) |> 
  pivot_longer(
    cols = c(rwc_class_65_70, rwc_class_70_80, rwc_class_80_90, 
             rwc_class_90, rwc_class_standard, rwc_class_tlp), 
    names_to = "rwc_class", values_to = "rwc_interval") |> 
  filter(rwc_interval != "other") |>
  nest(data = -c(year, campaign, sample_id, date, leaf_area_m2, rwc_class, rwc_interval)) 
...
df_gmin_slope_nest <- df_gmin_slope_nest |> 
  ...
  mutate(gmin_mean = - gmin_slope / leaf_area_m2 / mf_vpd,
...
df_gmin_params <- df_gmin_slope_nest |> 
  select(c(year, campaign, sample_id, date, rwc_class, rwc_interval, gmin_mean, gmin_min, gmin_max)) 


# export ------------------------------------------------------------------

write.csv(df_gmin_params, "data/calculated_parameters/df_gmin_params.csv", row.names = FALSE)
